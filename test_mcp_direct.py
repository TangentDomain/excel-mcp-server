...
async def test_mcp_tools():
    ...
    try:
        ...
        try:
            # 测试正常范围
            result1 = await excel_get_range("test_api_issues.xlsx", "A1:C3", include_formatting=False)
            print(f"   正常范围 (A1:C3): {len(result1.get('data', []))} 行数据")
            
            # 测试可能的颠倒参数 - 这里A1:C3已经是字符串表达式，没有顺序问题
            result2 = await excel_get_range("test_api_issues.xlsx", "C3:A1", include_formatting=False)
            print(f"   逆序范围 (C3:A1): {len(result2.get('data', []))} 行数据")
            
            if result1 == result2:
                print("   ⚠️  问题：范围表达式解析可能不够严格")
            else:
                print("   ✅ 范围表达式处理正确")
        ...
        try:
            ...
            result = await excel_format_cells("test_api_issues.xlsx", "Sheet1", "A1:A3", formatting=formatting)
            print("   ✅ 部分格式参数处理成功")
            
            # 测试不传格式参数
            result2 = await excel_format_cells("test_api_issues.xlsx", "Sheet1", "B1:B3")
            print("   ✅ 无格式参数处理成功")
        ...
        try:
            ...
            result = await excel_set_formula("test_api_issues.xlsx", "Sheet1", "A1")
        ...
        try:
            # 测试不存在的sheet - 应该报错
            result = await excel_search("test_api_issues.xlsx", "NonExistentSheet", "ID", "=1")
        ...
        # 测试有效的sheet
        try:
            result = await excel_search("test_api_issues.xlsx", "Sheet1", "ID", "=1")
            print(f"   ✅ 有效sheet搜索成功")
        ...
        try:
            ...
            result = await excel_update_range("test_api_issues.xlsx", "Sheet1", "D1", invalid_data)
        ...
        try:
            ...
            result = await excel_update_range("test_api_issues.xlsx", "Sheet1", "D1", valid_data)
            print("   ✅ 正确数据格式写入成功")
