...
def register_data_tools(mcp) -> None:
    ...
    @mcp.tool()
    def excel_get_range(file_path: str, range: str, include_formatting: bool = False) -> dict[str, Any]:
        """读取指定范围的单元格数据
        ...
        try:
            ...
            ExcelValidator.validate_operation_scale(range_validation["range_info"])
    ...
    @mcp.tool()
    def excel_update_range(file_path: str, range: str, data: Any, preserve_formulas: bool = True) -> dict[str, Any]:
        """更新指定范围的单元格数据
    ...
    @mcp.tool()
    def excel_get_headers(file_path: str, sheet_name: str, row: int = 1, max_columns: int = 50) -> dict[str, Any]:
        """获取工作表的表头信息
    ...
    @mcp.tool()
    def excel_find_last_row(file_path: str, sheet_name: str | None = None, column: str | None = None) -> dict[str, Any]:
        """查找工作表中最后一行数据的位置
    ...
    @mcp.tool()
    def excel_insert_rows(file_path: str, sheet_name: str, row: int, count: int = 1) -> dict[str, Any]:
        """插入行
    ...
    @mcp.tool()
    def excel_delete_rows(file_path: str, sheet_name: str, row: int, count: int = 1) -> dict[str, Any]:
        """删除行
    ...
    @mcp.tool()
    def excel_insert_columns(file_path: str, sheet_name: str, column: int, count: int = 1) -> dict[str, Any]:
        """插入列
    ...
    @mcp.tool()
    def excel_delete_columns(file_path: str, sheet_name: str, column: int, count: int = 1) -> dict[str, Any]:
        """删除列
