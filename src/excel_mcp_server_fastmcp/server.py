...
from .utils.validators import DataValidationError, ExcelValidator
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if isinstance(result, dict) and result.get("success") is False:
                ...
                _tracker.record(func.__name__, duration_ms, success=False, error_type=error_type)
                logger.debug(
                    f"[TOOL] {func.__name__}: {duration_ms:.1f}ms ERROR [{error_type}]",
                    extra={
                        "tool": func.__name__,
                ...
                logger.debug(
                    f"[TOOL] {func.__name__}: {duration_ms:.1f}ms",
            ...
            _tracker.record(func.__name__, duration_ms, success=False, error_type=error_type)
            logger.debug(
                f"[TOOL] {func.__name__}: {duration_ms:.1f}ms ERROR [{error_type}]: {e}",
                extra={
                    "tool": func.__name__,
                    "duration_ms": round(duration_ms, 1),
                    "error": str(e),
...
class SecurityValidator:
    ...
    def validate_file_path(cls, file_path: str) -> dict[str, Any]:
        ...
        if ext and ext.lower() not in cls.ALLOWED_EXTENSIONS:
            return {
                "valid": False,
                "error": f"不支持的文件格式: {ext}（允许: {', '.join(sorted(cls.ALLOWED_EXTENSIONS))}）",
            }
    ...
    def validate_file_size(cls, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if size > cls.MAX_FILE_SIZE:
                return {
                    "valid": False,
                    "error": f"文件过大: {size / 1024 / 1024:.1f}MB（上限{cls.MAX_FILE_SIZE / 1024 / 1024:.0f}MB）",
                }
            return {"valid": True, "error": None}
        except OSError as e:
    ...
    def validate_formula(cls, formula: str) -> dict[str, Any]:
        ...
        for pattern in cls.DANGEROUS_FORMULA_PATTERNS:
            if pattern.search(formula):
                return {
                    "valid": False,
                    "error": f"检测到危险公式模式，公式中不允许包含 {pattern.pattern}",
                }
        return {"valid": True, "error": None}
...
def _validate_path(file_path: str) -> dict[str, Any] | None:
...
_ERROR_HINTS = {
    ...
    "MISSING_PARAM": "请检查必填参数是否已提供。查看工具描述了解所有必填参数。",
    "MISSING_REQUIRED_PARAM": "请提供必填参数。查看工具描述了解参数格式和示例。",
    "MISSING_CODE": "code参数不能为空。请提供要执行的Python代码。",
    "MISSING_FORMULA": "formula参数不能为空。请提供以=开头的Excel公式，如=SUM(A1:A10)。",
    "MISSING_FORMATTING_PARAMS": "请提供格式化参数。查看excel_format_cells工具描述了解支持的格式选项。",
    "INVALID_FORMAT": "请检查参数值是否在允许范围内。查看工具描述了解支持的选项。",
    "INVALID_PARAMETER": "参数值不合法。请查看工具描述了解参数格式和取值范围。",
    "INVALID_OPERATION": "不支持的操作类型。请查看工具描述了解支持的operation值。",
    ...
    "BACKUP_NOT_FOUND": "备份文件不存在。请用excel_backup的list操作查看可用的备份列表。",
    "RESTORE_FAILED": "恢复失败。备份文件可能已损坏。请用excel_backup的list操作选择其他备份。",
    "LIST_BACKUPS_FAILED": "获取备份列表失败。请检查备份目录是否存在。",
    "DELETE_SHEET_FAILED": "删除工作表失败。文件可能被锁定或工作表名不正确。",
    "FORMAT_FAILED": "格式设置失败。请确认单元格范围、样式参数正确。",
    ...
    "INSERT_EXECUTION_FAILED": "INSERT执行失败。请检查SQL语法，确保表名、列名和VALUES正确。",
    "UPDATE_EXECUTION_FAILED": "UPDATE执行失败。请检查SQL语法，确保SET和WHERE子句正确。可用dry_run=True预览。",
    "DELETE_EXECUTION_FAILED": "DELETE执行失败。请检查SQL语法，确保WHERE条件正确（不允许无WHERE的DELETE）。",
    "SCRIPT_ERROR": "Python脚本执行失败。请检查代码语法和逻辑。可用变量: query(), update(), insert(), delete()。",
...
def excel_search_directory(
    ...
    *,
    ...
    if use_regex is None:
        use_regex = bool(re.match(r".*[\[\](){}*+?|^$\\.]", pattern))
    return _wrap(
        ExcelOperations.search_directory(
            directory_path,
            pattern,
            case_sensitive,
            whole_word,
        ...
        if "!" not in cell_range:
            try:
                ...
                if len(sheet_names) == 1:
                    ...
                    return _fail(
                        f"范围表达式缺少工作表名，且文件有多个工作表({', '.join(sheet_names)})。请使用格式: '工作表名!A1:C10'",
                        meta={
...
def excel_update_range(
    ...
    try:
        ...
        operation_logger.log_operation(
            "validation_failed",
            {"operation": "update_range", "cell_range": cell_range, "error": str(e)},
        )
    ...
    try:
        ...
        # 记录错误
        operation_logger.log_operation("operation_error", {"error": str(e), "message": f"更新操作失败: {str(e)}"})
...
def excel_query(
    ...
    if output_format and output_format not in valid_formats:
        return _fail(
            f"不支持的输出格式: {output_format}。可选: {', '.join(valid_formats)}",
            meta={"error_code": "INVALID_FORMAT"},
        )
    ...
    try:
        ...
        return _fail(hint, meta={"error_code": "DEPENDENCY_MISSING"})
    except Exception as e:
...
def excel_update_query(file_path: str, update_expression: str, dry_run: bool = False) -> dict[str, Any]:
    ...
    if not stripped.startswith("UPDATE"):
        return _fail(
            "只支持UPDATE语句。查询请使用 excel_query",
            meta={"error_code": "UNSUPPORTED_SQL"},
        )

    try:
        from .api.advanced_sql_query import execute_advanced_update_query

        return _wrap(execute_advanced_update_query(file_path=file_path, sql=update_expression, dry_run=dry_run))
    except ImportError:
        return _fail(
            "SQLGlot未安装，无法使用UPDATE功能",
            meta={"error_code": "DEPENDENCY_MISSING"},
        )
    except Exception as e:
...
def excel_insert_query(file_path: str, insert_expression: str, dry_run: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return _wrap(execute_advanced_insert_query(file_path=file_path, sql=insert_expression, dry_run=dry_run))
    except ImportError:
        return _fail(
            "SQLGlot未安装，无法使用INSERT功能",
            meta={"error_code": "DEPENDENCY_MISSING"},
        )
    except Exception as e:
...
def excel_delete_query(file_path: str, delete_expression: str, dry_run: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return _wrap(execute_advanced_delete_query(file_path=file_path, sql=delete_expression, dry_run=dry_run))
    except ImportError:
        return _fail(
            "SQLGlot未安装，无法使用DELETE功能",
            meta={"error_code": "DEPENDENCY_MISSING"},
        )
    except Exception as e:
...
def _prepare_describe_result(sheet_name, is_dual_header, columns, row_count, file_path, sheet):
    ...
    """
    return _ok(
        f"表 '{sheet_name}': {len(columns)}列, {row_count}行数据, {'双行表头' if is_dual_header else '单行表头'}",
        data={
...
def _check_merge_data_loss(file_path: str, sheet_name: str, cell_range: str) -> str | None:
