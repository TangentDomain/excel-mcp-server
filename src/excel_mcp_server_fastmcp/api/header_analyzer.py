...
class HeaderInfo:
    ...
    def resolve_column(self, key: str) -> Optional[int]:
        """
...
def _cell_str(c: Any) -> Optional[str]:
    """统一单元格值转字符串"""
    if c is None:
        return None
    if hasattr(c, 'value'):
...
def detect_from_rows(rows: List[List]) -> Tuple[bool, int, List[str]]:
    ...
    # 第二行是否全是有效英文字段名
    all_valid_names = all(
        _is_valid_field_name(v) for v in second_non_empty
    )
    
    # 第一行是否有中文（或者第一行不是纯英文字段名格式）
    any_chinese = any(
        _contains_chinese(v) for v in first_non_empty
    )
    
    # 如果第二行全是有效英文名且第一行有中文 → 双表头
    # 额外条件：第一行不能也全是有效英文名（否则可能是两层都是英文名）
    first_all_valid = all(
        _is_valid_field_name(v) for v in first_non_empty
    ) if first_non_empty else False
...
class HeaderAnalyzer:
    ...
    @classmethod
    def analyze(cls, file_path: str, sheet_name: str,
                force_refresh: bool = False) -> HeaderInfo:
        """
        分析指定工作表的表头结构（带缓存）。
    ...
    @classmethod
    def invalidate(cls, file_path: Optional[str] = None):
        """
        使缓存失效。
    ...
    @classmethod
    def get_effective_header_row(cls, file_path: str, sheet_name: str,
                                  explicit_header_row: Optional[int] = None) -> int:
        """
        获取有效的表头行号。
    ...
    @classmethod
    def get_data_start_row(cls, file_path: str, sheet_name: str,
                           explicit_header_row: Optional[int] = None) -> int:
        """
        获取数据起始行号。
    ...
    @classmethod
    def resolve_key_to_column(cls, file_path: str, sheet_name: str,
                               key: str, explicit_header_row: Optional[int] = None) -> Optional[int]:
        """
    ...
    def summary_text(cls, info: HeaderInfo) -> str:
        """生成人类可读的表头摘要"""
        if info.is_dual:
            return (f"双行表头(第{info.header_rows[0]}-{info.header_rows[1]}行), "
                    f"数据从第{info.data_start_row}行开始, "
                    f"共{info.total_columns}列")
        else:
            return (f"单行表头(第{info.header_rows[0]}行), "
                    f"数据从第{info.data_start_row}行开始, "
                    f"共{info.total_columns}列")
