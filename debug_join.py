...
def debug_join():
    ...
    try:
        ...
        # 检查引擎的JOIN列映射
        print(f"\n引擎JOIN列映射: {getattr(engine, '_join_column_mapping', '无')}")
        print(f"引擎表别名映射: {getattr(engine, '_table_aliases', '无')}")
        
        # 查看JOIN后的DataFrame结构
        worksheets_data = engine._load_data_with_cache(test_file)
        df = worksheets_data["角色"]
        
        print(f"\n角色表列名: {list(df.columns)}")
        
        # 执行JOIN并查看中间结果
        try:
            parsed_sql = engine._parse_sql(query)
            df_result = engine._execute_query(parsed_sql, worksheets_data)
            
            print(f"\nJOIN后的DataFrame列名: {list(df_result.columns)}")
            print(f"JOIN后的DataFrame形状: {df_result.shape}")
