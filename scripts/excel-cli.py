#!/usr/bin/env python3
"""ExcelMCP CLI — skill 可执行文件。

将 MCP 工具转为 CLI 子命令，输出 JSON。支持 self-update 从 GitHub 拉最新版本。
用法: python scripts/excel-cli.py <command> [options]
"""
import sys
import os
import json
...
from excel_mcp_server_fastmcp.utils.validators import ExcelValidator

VERSION = "1.0.0"
GITHUB_REPO = "TangentDomain/excel-mcp-server"
# PyInstaller 打包后 __file__ 为冻结路径；源码模式直接拉 .py
...
def cmd_self_update(args):
    """从 GitHub 拉取最新版本并替换自身。

    源码模式（.py）: 下载 GitHub raw 源码并原子替换
    二进制模式（PyInstaller）: 下载对应平台预编译二进制
    """
    try:
        platform, arch = detect_platform()
        self_path = os.path.abspath(__file__)
        # PyInstaller 冻结模式下 sys.executable 是二进制自身
        if _IS_FROZEN:
        ...
        if not _IS_FROZEN:
            try:
                ...
                pass
        # 二进制模式版本从 URL 或 release tag 无法直接提取，暂标记为 unknown
...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog="示例:\n"
               "  python excel-cli.py list-sheets --file data.xlsx\n"
               "  python excel-cli.py query --file data.xlsx --sql \"SELECT * FROM Sheet1 LIMIT 5\"\n"
               "  python excel-cli.py update-query --file data.xlsx --sql \"UPDATE Sheet1 SET Col=1 WHERE ID>5\"\n"
               "  python excel-cli.py self-update",
    ...
    # ---- self-update ----
    p = subparsers.add_parser("self-update", help="从 GitHub 更新 CLI 到最新版本")
