...
### REQ-007 [P1] README文档同步更新
- **来源**：代码已实现但README未同步
- **描述**：README中多处信息过时，需要同步最新功能
- **已知过时内容**：
  - FAQ中"JOIN不支持"→ 已支持INNER/LEFT JOIN
  - 测试数badge 1166→ 实际130+
  - 工具数badge可能未更新（现在41个）
  - SQL参考部分缺JOIN示例和UPDATE示例
  - 不支持列表中JOIN/子查询→ JOIN已支持，需更新
- **验收**：README中所有功能描述与代码实际能力一致
- **状态**：OPEN
