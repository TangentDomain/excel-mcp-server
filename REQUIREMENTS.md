...
- **来源**：功能盘点 — SQL引擎唯一剩余的查询能力缺口，CTE可替代但语法不完全等价
- **描述**：支持 `SELECT * FROM (SELECT ...) AS t WHERE ...` 语法，解析FROM子句中的嵌套SELECT
- **验收标准**：基础FROM子查询 + WHERE条件过滤 + JOIN结果子查询 + 嵌套子查询，至少4个测试
- **状态**：OPEN
