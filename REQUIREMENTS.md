...
## 测试报告问题（极限测试 2026-04-09）

### REQ-EXCEL-020 (P0): excel_evaluate_formula 极慢（295秒）
- **来源**: 用户极限测试报告 §7.1
- **描述**: `excel_evaluate_formula` 计算 `=SUM(1,2,3,4,5)` 耗时约295秒，完全不可用。需排查是否依赖外部进程/COM调用，考虑缓存或替换实现方案。
- **状态**: OPEN

### REQ-EXCEL-021 (P1): excel_create_chart 文件锁定失败
- **来源**: 用户极限测试报告 §7.2
- **描述**: 创建图表时文件锁定失败，可能是因为写入后未正确释放锁或Windows下文件句柄未关闭。需确保操作完成后释放文件锁，或增加重试机制。
- **状态**: OPEN

### REQ-EXCEL-022 (P2): excel_batch_insert_rows 需要表头才能工作
- **来源**: 用户极限测试报告 §7.3
- **描述**: 空表插入数据时报"未找到表头列名"。应自动创建表头或允许指定列名列表，而不是强制要求先手动建表头。
- **状态**: OPEN

### REQ-EXCEL-023 (P2): excel_insert_columns 验证逻辑过严
- **来源**: 用户极限测试报告 §7.4
- **描述**: 插入列时验证失败，可能检查了不存在的约束（如表范围）。应放宽验证或修复验证逻辑与实际操作的匹配。
- **状态**: OPEN
