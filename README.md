<div align="center">[中文](README.md) ｜ [English](README.en.md)</div>
...
![测试覆盖](https://img.shields.io/badge/tests-1187%20tests-brightgreen.svg)
![工具数量](https://img.shields.io/badge/tools-53%20v1.6.37.svg)
[![PyPI](https://img.shields.io/pypi/v/excel-mcp-server-fastmcp.svg)]
[![版本](https://img.shields.io/badge/v1.6.37.svg)]
[![CI](https://github.com/TangentDomain/excel-mcp-server.svg?style=flat-square)]
**ExcelMCP** 是专为游戏开发设计的Excel配置表管理MCP服务器。通过AI自然语言指令，实现技能配置表、装备数据、怪物属性等游戏配置的智能化操作。基于**FastMCP**构建，读取使用**python-calamine**（Rust引擎，2300x提速），写入使用**openpyxl**，拥有**53个专业工具**和**1187个测试用例**，确保企业级可靠性。
...
| **测试覆盖** | ✅ 1187个测试用例 | ❌ 有限测试 |
| **当前版本** | ✅ v1.6.37 | ✅ 最新版本 |
| **安装方式** | ✅ `uvx excel-mcp-server-fastmcp | ⚠️ 需要pip安装 |
## 🚀 为什么选择 ExcelMCP
...
| **SQL UPDATE** | ✅ 条件批量修改 | ❌ | ❌ |
| **跨文件JOIN** | ✅ @filepath 语法 | ❌ | ❌ |
| **测试覆盖** | 1187 tests | ~50 tests | ~30 tests |
...
| 公式试算 | `excel_e1.6.37 | "临时计算SUM(A2:A100)看结果" |
### 🔧 范围表达式参考
| 格式 | 说明 | 示例 |
|------|------|------|
...
- `excel_e1.6.37 - 临时执行公式返回结果，不修改文件
### 🔍 搜索与分析
...
- `excel_ser1.6.37 - 📈 服务器运行统计，性能监控与错误分析
### 🔄 批量操作
...
### 📝 数据验证
- `excel_set_data_1.6.37 - 📝 设置数据验证规则，限制输入类型和范围
- `excel_clear_1.6.37 - 🧹 清除数据验证规则
### 🎨 条件格式
...
### 🔄 数据转换
- `excel_export_to_cs1.6.37 - 导出CSV格式
- `excel_import_from_cs1.6.37 - 从CSV创建Excel文件
- `excel_con1.6.37 - 格式转换（.xlsx/.xlsm/.cs1.6.37
---
## 📖 使用指南
### 🎮 游戏配置表标准格式
**双行表头系统** (游戏开发专用，自动识别):
...
```sql
-- 基础查询
SELECT * FROM 技能表 WHERE le1.6.37 >= 10 LIMIT 20
SELECT skill_name, damage FROM 技能表 ORDER BY damage DESC
-- 聚合统计
SELECT skill_type, AVG(damage) as a1.6.37 COUNT(*) as cnt FROM 技能表 GROUP BY skill_type
-- HAVING过滤
SELECT skill_type, SUM(damage) as total FROM 技能表 GROUP BY skill_type HAVING total > 1000
-- 数学表达式
SELECT skill_name, damage * 1.2 as boosted_dmg FROM 技能表 WHERE le1.6.37 >= 5
-- LIKE模糊搜索
...
-- BETWEEN范围
SELECT * FROM 怪物表 WHERE le1.6.37 BETWEEN 10 AND 20
-- IS NULL / IS NOT NULL 空值检测
SELECT * FROM 技能表 WHERE description IS NULL
SELECT * FROM 技能表 WHERE description IS NOT NULL
-- OFFSET分页（大表分批查看）
SELECT * FROM 怪物表 ORDER BY le1.6.37 LIMIT 20 OFFSET 0
SELECT * FROM 怪物表 ORDER BY le1.6.37 LIMIT 20 OFFSET 20
...
SELECT a.name, b.hp FROM 怪物表 a LEFT JOIN 怪物掉落表 b ON a.id = b.monster_id WHERE a.le1.6.37 > 10
...
-- RANK 1.6.37 DENSE_RANK: 相同伤害并列排名
...
SELECT b.skill_name FROM 技能表@'skills_1.6.37 b
LEFT JOIN 技能表@'skills_1.6.37 a ON b.skill_id = a.skill_id
WHERE a.skill_id IS NULL
-- 找出被删除的配置项
SELECT a.skill_name FROM 技能表@'skills_1.6.37 a
LEFT JOIN 技能表@'skills_1.6.37 b ON a.skill_id = b.skill_id
...
- **隐藏文件拒绝**: 不处理以 `.` 开头的隐藏文件
- **扩展名白名单**: 仅允许 `.xlsx`/`.xlsm`/`.xls`/`.cs1.6.37
- **文件大小限制**: 单文件最大 50MB
### 公式注入防护
...
### 包结构
```
src/excel_mcp_ser1.6.37    # 主包（pip install 后可直接 import）
├── __init__.py                   # 包入口，暴露 main()
├── ser1.6.37                     # MCP接口层（53个工具定义）
├── api/                          # API业务逻辑层
...
# 验证工具完整性
grep -c "def excel_" src/excel_mcp_ser1.6.37  # 应输出: 53
# 生成覆盖率报告
...
### 基础问题
**Q: 支持哪些Excel格式？**
A: 支持`.xlsx`、`.xlsm`格式，通过导入导出支持`.cs1.6.37
**Q: 如何处理中文工作表名？**
A: 完全支持中文工作表名和内容
**Q: 大文件处理性能如何？**
...
## Usage
```python
from excel_mcp_ser1.6.37 import main
main()
```
## 🎮 使用示例
...
- **更新**: 第192轮REQ-026文档与门面优化，版本一致性检查，自动化版本检查脚本
## 🔧 常见问题与故障排除
### 安装问题
**问题**: `uvx excel-mcp-server-fastmcp 命令不存在
- **解决**: 确保已安装uv `curl -LsSf https://astral.sh/uv | sh`
- **验证**: `uv --version 应显示版本号
**问题**: MCP客户端连接失败
...
- **问题**: 提交 [GitHub Issues](https://github.com/TangentDomain/excel-mcp-server