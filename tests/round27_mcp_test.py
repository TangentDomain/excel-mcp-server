...
def create_test_files():
    ...
    for i in range(1, 31):
        ws_equip.append([i, f"Item-{i}", random.randint(10, 100), round(random.uniform(5.5, 45.8), 2), round(random.uniform(50.5, 9999.99), 2), random.choice(rarities), random.choice(categories)])

    # Sheet 2: 怪物配置
    ws_monster = wb.create_sheet("怪物配置")
    ws_monster.append(["ID", "Name", "Level", "HP", "ZoneID"])
    for i in range(1, 21):
        ws_monster.append([i, f"Monster-{i}", random.randint(1, 99), random.randint(100, 9999), random.randint(1, 5)])

    # Sheet 3: 掉落配置
    ws_drop = wb.create_sheet("掉落配置")
    ws_drop.append(["DropID", "MonsterID", "ItemID", "DropRate", "MinQty", "MaxQty"])
    for i in range(1, 41):
        ws_drop.append([i, random.randint(1, 20), random.randint(1, 30), round(random.uniform(0.01, 1.0), 4), random.randint(1, 3), random.randint(3, 10)])
...
class _TestResult:
    ...
    def add(self, name, category, passed, detail=""):
        ...
        self.results.append({"name": name, "category": category, "passed": passed, "detail": detail})
        print(f"  {status} | {name}")
...
# 导入 API
# ============================================================
from excel_mcp_server_fastmcp.api.advanced_sql_query import execute_advanced_delete_query, execute_advanced_insert_query, execute_advanced_sql_query, execute_advanced_update_query
...
tr.add("A1: 三层CTE嵌套(聚合→窗口函数→过滤)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A2: 四层CTE嵌套(聚合→窗口→过滤→计算)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A3: SELECT中相关子查询(scalar subquery)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A4: WHERE IN 子查询", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A5: EXISTS 相关子查询", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A6: NOT EXISTS 子查询(无掉落的装备)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A7: 三层标量子查询嵌套(WHERE>SELECT>SELECT)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A8: CTE+JOIN+窗口函数(PARTITION+ROW_NUMBER)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A9: CASE WHEN嵌套在SUM聚合中(GROUP BY)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("A10: CTE包装(HAVING+窗口函数)", "深度嵌套", r.get("success", False), f"rows={len(r.get('data', [])) - 1}" if r.get("success") else r.get("message", "")[:100])
...
tr.add("B1a: UPDATE SET单行", "数据一致性", r_upd.get("success", False), r_upd.get("message", "")[:100] if not r_upd.get("success") else "")

r_after = run_query(fs, "SELECT Value FROM Sheet1 WHERE ID = 1")
new_val = r_after["data"][1][0] if len(r_after.get("data", [])) > 1 else None
tr.add("B1b: UPDATE后验证值=999", "数据一致性", new_val == 999, f"期望=999 实际={new_val}")
...
tr.add("B2a: UPDATE全表(Score+1)", "数据一致性", r_upd2.get("success", False), r_upd2.get("message", "")[:100])
...
tr.add("B2b: UPDATE后总行数不变", "数据一致性", before_total == after_total, f"UPDATE前={before_total} UPDATE后={after_total}")
...
tr.add("B3a: INSERT新行(ID=999)", "数据一致性", r_ins.get("success", False), r_ins.get("message", "")[:100])

r_verify = run_query(fs, "SELECT * FROM Sheet1 WHERE ID = 999")
found = len(r_verify.get("data", [])) > 1  # header + data row
tr.add("B3b: INSERT后能查询到ID=999", "数据一致性", found, f"查询到{len(r_verify.get('data', [])) - 1}行" if r_verify.get("data") else "空结果")

if found:
    vals = r_verify["data"][1]
    tr.add("B3c: INSERT数据值正确(999,777,88.8)", "数据一致性", vals[0] == 999 and vals[1] == 777 and abs(float(vals[2]) - 88.8) < 0.01, f"实际值={vals}")
...
tr.add("B4a: DELETE已插入的行", "数据一致性", r_del.get("success", False), r_del.get("message", "")[:100])

r_check = run_query(fs, "SELECT * FROM Sheet1 WHERE ID = 999")
deleted_ok = len(r_check.get("data", [])) <= 1  # only header
tr.add("B4b: DELETE后确认不存在", "数据一致性", deleted_ok, f"还剩{len(r_check.get('data', [])) - 1 if r_check.get('data') else 0}行")
...
tr.add("B5a: 批量UPDATE(Value*2)成功", "数据一致性", r_b5_upd.get("success", False), r_b5_upd.get("message", "")[:100])
tr.add("B5b: UPDATE后SUM翻倍", "数据一致性", abs(post_sum - pre_sum * 2) < 0.01, f"前SUM={pre_sum:.1f} 后SUM={post_sum:.1f} 期望={pre_sum * 2:.1f}")
tr.add("B5c: UPDATE后COUNT不变", "数据一致性", pre_cnt == post_cnt, f"前={pre_cnt} 后={post_cnt}")
...
tr.add("C1: 不存在表的错误提示", "错误质量", has_error_msg and safe and len(msg) > 0, f"msg='{msg[:150]}'" if msg else "无错误消息(!)")
...
tr.add("C2: 不存在列的错误提示", "错误质量", has_error_msg and safe, f"msg='{msg[:150]}'" if msg else "无错误消息")

# C3: SQL语法错误
r = run_query(f, "SELEC * FORM 装备配置")  # 故意拼写错误
has_error_msg = not r.get("success", True)
msg = r.get("message", "")
tr.add("C3: SQL语法错误的提示", "错误质量", has_error_msg and len(msg) > 0, f"msg='{msg[:150]}'" if msg else "无错误消息")
...
# C6: 空SQL
r = run_query(f, "")
tr.add("C6: 空SQL字符串", "错误质量", not r.get("success", True), f"msg='{str(r.get('message', ''))[:100]}'")
...
tr.add("C9: NULL字面量选择", "错误质量", r.get("success", False) or r.get("success", True), f"success={r.get('success')} data={str(r.get('data', ''))[:100]}")
...
tr.add("E1: LIKE前缀匹配(Item-1%)", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E2: LIKE 通配符
r = run_query(f, "SELECT * FROM 装备配置 WHERE Name LIKE 'Item-_3' ORDER BY ID")
tr.add("E2: LIKE单字符通配符(_)", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E3: BETWEEN
r = run_query(f, "SELECT * FROM 装备配置 WHERE BaseAtk BETWEEN 30 AND 60 ORDER BY BaseAtk")
tr.add("E3: BETWEEN范围查询", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E4: IN 列表
r = run_query(f, "SELECT * FROM 装备配置 WHERE ID IN (1, 3, 5, 7, 9) ORDER BY ID")
tr.add("E4: IN列表(显式值)", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E5: NOT IN
r = run_query(f, "SELECT * FROM 装备配置 WHERE ID NOT IN (1, 2, 3) ORDER BY ID LIMIT 5")
tr.add("E5: NOT IN列表", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E6: DISTINCT
r = run_query(f, "SELECT DISTINCT Rarity FROM 装备配置 ORDER BY Rarity")
tr.add("E6: DISTINCT去重", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E7: IS NULL / IS NOT NULL (如果有NULL数据)
r = run_query(f, "SELECT * FROM 装备配置 WHERE Rarity IS NOT NULL LIMIT 3")
tr.add("E7: IS NOT NULL判断", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E8: ORDER BY 多列
r = run_query(f, "SELECT * FROM 装备配置 ORDER BY Rarity, Price DESC LIMIT 5")
tr.add("E8: ORDER BY多列(Rarity ASC, Price DESC)", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")

# E9: LIMIT OFFSET
r = run_query(f, "SELECT * FROM 装备配置 ORDER BY ID LIMIT 3 OFFSET 5")
tr.add("E9: LIMIT 3 OFFSET 5分页", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("E10: 复杂AND/OR布尔条件组合", "特殊场景", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("F1: 三表JOIN(装备+掉落+怪物)", "跨表JOIN", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("F2: LEFT JOIN(含无掉落装备)", "跨表JOIN", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("F3: JOIN+GROUP BY聚合统计", "跨表JOIN", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("F4: 自连接(同表稀有度比较)", "跨表JOIN", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
...
tr.add("F5: CROSS JOIN(笛卡尔积+子查询)", "跨表JOIN", r.get("success", False), f"rows={len(r.get('data', [])) - 1 if r.get('data') else 0}")
