...
class TestCrossFileBasicJoin:
    ...
    def test_inner_join_same_directory(self, engine, skills_file, drops_file):
        """同目录跨文件INNER JOIN"""
        sql = f"SELECT s.技能名称, d.掉落物品 FROM 技能配置@'{skills_file}' s JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能"
    ...
    def test_left_join_preserves_all_rows(self, engine, skills_file, drops_file):
        ...
        sql = f"SELECT s.技能名称, d.掉落物品 FROM 技能配置@'{skills_file}' s LEFT JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
        # 所有4个技能都应该出现
    ...
    def test_cross_join(self, engine, skills_file, drops_file):
        """跨文件CROSS JOIN（笛卡尔积）"""
        sql = f"SELECT s.技能名称, d.掉落物品 FROM 技能配置@'{skills_file}' s CROSS JOIN 掉落配置@'{drops_file}' d"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
    ...
    def test_where_on_cross_file_join(self, engine, skills_file, drops_file):
        """跨文件JOIN + WHERE过滤"""
        sql = f"SELECT s.技能名称, d.掉落物品 FROM 技能配置@'{skills_file}' s JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能 WHERE s.技能类型 = '法师'"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
        # 只有2个法师技能
    ...
    def test_order_by_on_cross_file_join(self, engine, skills_file, drops_file):
        """跨文件JOIN + ORDER BY排序"""
        sql = f"SELECT s.技能名称, d.数量 FROM 技能配置@'{skills_file}' s JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能 ORDER BY d.数量 DESC"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
    ...
    def test_limit_on_cross_file_join(self, engine, skills_file, drops_file):
        """跨文件JOIN + LIMIT限制"""
        sql = f"SELECT s.技能名称, d.掉落物品 FROM 技能配置@'{skills_file}' s JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能 LIMIT 2"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
        assert len(data) == 3  # header + 2 data rows
...
class TestCrossFileDifferentDirectory:
    ...
    def test_relative_path_from_primary_file(self, engine, skills_file, subfile):
        ...
        sql = f"SELECT s.技能名称, e.装备名 FROM 技能配置@'{subfile}' s JOIN 装备表@'{subfile}' e ON s.技能ID = e.关联技能"
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
        assert len(data) == 3  # header + 2 data rows
...
class TestCrossFileThreeWayJoin:
    ...
    def test_three_way_join(self, engine, skills_file, drops_file, monsters_file):
        """三表跨文件JOIN"""
        sql = (
            f"SELECT s.技能名称, d.掉落物品, m.怪物名 FROM 技能配置@'{skills_file}' s JOIN 掉落配置@'{drops_file}' d ON s.技能ID = d.关联技能 JOIN 怪物表@'{monsters_file}' m ON s.技能ID = m.关联技能"
        )
        result = engine.execute_sql_query(skills_file, sql)
        assert result["success"] is True
        data = result["data"]
