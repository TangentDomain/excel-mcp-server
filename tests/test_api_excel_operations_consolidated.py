...
class TestExcelOperationsAdvanced:
    ...
    def test_large_data_insert(self, test_file):
        """测试大量数据插入"""
        # 插入100行数据
        data = [{"ID": i, "Name": f"Item{i}", "Value": i * 10, "Category": "Test", "Status": "Active"} for i in range(100, 200)]
    ...
    def test_batch_insert_performance(self, test_file):
        ...
        # 插入50行数据
        data = [{"ID": i, "Name": f"PerfItem{i}", "Value": i * 100, "Category": "Perf", "Status": "Active"} for i in range(1000, 1050)]
