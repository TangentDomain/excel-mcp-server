...
class TestInsert:
    def test_insert_single_row(self, raids_file):
        ...
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (RID, CID, Name, Score) VALUES (6, 105, 'fire', 70)")
    ...
    def test_insert_multi_row(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (RID, Score) VALUES (6, 70), (7, 80)")
        assert result["success"] is True
        assert result["affected_rows"] == 2

    def test_insert_dry_run(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (RID, Score) VALUES (6, 70)", dry_run=True)
        assert result["success"] is True
    ...
    def test_insert_wrong_column(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (NonExist, Score) VALUES (1, 70)")
        assert result["success"] is False
        assert "不存在" in result["message"]

    def test_insert_wrong_table(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO NonExist (RID) VALUES (1)")
        assert result["success"] is False
        assert "不存在" in result["message"]

    def test_insert_null_value(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (RID, Name, Score) VALUES (6, NULL, 70)")
        assert result["success"] is True

    def test_insert_negative_value(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_insert_query(raids_file, "INSERT INTO Raids (RID, Score) VALUES (6, -5)")
        assert result["success"] is True
...
class TestDelete:
    def test_delete_with_where(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids WHERE Score < 80")
        assert result["success"] is True
        assert result["affected_rows"] == 1  # RID=3, Score=78

    def test_delete_with_row_number(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids WHERE _ROW_NUMBER_ IN (1, 3)")
        assert result["success"] is True
        assert result["affected_rows"] == 2

    def test_delete_no_match(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids WHERE Score > 200")
        assert result["success"] is True
        assert result["affected_rows"] == 0

    def test_delete_dry_run(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids WHERE Score < 80", dry_run=True)
        assert result["success"] is True
        assert result["dry_run"] is True
        assert result["affected_rows"] == 1
    ...
    def test_delete_without_where(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids")
        assert result["success"] is False
        assert "WHERE" in result["message"]

    def test_delete_wrong_table(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM NonExist WHERE Score > 0")
        assert result["success"] is False

    def test_delete_with_complex_condition(self, raids_file):
        engine = AdvancedSQLQueryEngine()
        result = engine.execute_delete_query(raids_file, "DELETE FROM Raids WHERE Name = 'ice' AND Score > 90")
        assert result["success"] is True
...
class TestCrossFileJoin:
    def test_cross_file_join(self, tmp_path):
        ...
        pd.DataFrame({"PID": [1, 2, 3], "Guild": [100, 100, 200]}).to_excel(file1, index=False, sheet_name="Players")

        pd.DataFrame({"CID": [101, 102, 103], "PID": [1, 1, 2], "ILv": [450, 440, 420]}).to_excel(file2, index=False, sheet_name="Chars")
