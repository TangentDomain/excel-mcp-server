...
class TestHavingChineseColumnSuggestion:
    ...
    def test_having_cn_with_explicit_alias_gt(self):
        ...
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, AVG(伤害) as avg_dmg FROM 技能配置 GROUP BY 技能类型 HAVING AVG(伤害) > 99999")
    ...
    def test_having_cn_with_explicit_alias_lt(self):
        ...
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, COUNT(*) as cnt FROM 技能配置 GROUP BY 技能类型 HAVING COUNT(*) < 1")
        assert result["success"] is True
        suggestion = result["query_info"].get("suggestion", "")
    ...
    def test_having_cn_with_explicit_alias_sum_gte(self):
        ...
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, SUM(伤害) as total FROM 技能配置 GROUP BY 技能类型 HAVING SUM(伤害) >= 99999")
        assert result["success"] is True
        suggestion = result["query_info"].get("suggestion", "")
    ...
    def test_having_cn_without_alias(self):
        ...
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, AVG(伤害) FROM 技能配置 GROUP BY 技能类型 HAVING AVG(伤害) > 99999")
        assert result["success"] is True
        suggestion = result["query_info"].get("suggestion", "")
    ...
    def test_having_cn_count_star_no_alias(self):
        ...
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, COUNT(*) FROM 技能配置 GROUP BY 技能类型 HAVING COUNT(*) > 999")
        assert result["success"] is True
        suggestion = result["query_info"].get("suggestion", "")
    ...
    def test_having_no_empty_result_no_suggestion(self):
        """HAVING有结果时不生成空结果建议"""
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT 技能类型, AVG(伤害) as avg_dmg FROM 技能配置 GROUP BY 技能类型 HAVING AVG(伤害) > 50")
        assert result["success"] is True
    ...
    def test_having_en_column_still_works(self):
        """HAVING英文列名仍正常（回归测试）"""
        result = execute_advanced_sql_query(GAME_CONFIG, "SELECT skill_type, AVG(damage) as avg_dmg FROM 技能配置 GROUP BY skill_type HAVING AVG(damage) > 99999")
        assert result["success"] is True
        suggestion = result["query_info"].get("suggestion", "")
