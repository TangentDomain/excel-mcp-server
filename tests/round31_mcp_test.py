...
def record(tool_name, input_desc, expected, actual, passed, notes=""):
    ...
    RESULTS.append({"tool": tool_name, "input": input_desc, "expected": expected, "actual": str(actual)[:100], "passed": passed, "notes": notes})
...
def test_group_a_big_data_stress():
    ...
    # A3: 50K行GROUP BY聚合
    t0 = time.time()
    result = execute_advanced_sql_query(path_50k, "SELECT CASE WHEN ID <= 25000 THEN 'A' ELSE 'B' END as grp, COUNT(*) as cnt, SUM(Value) as total FROM Sheet1 GROUP BY grp")
    t1 = time.time()

    record("excel_query", "50K行GROUP BY聚合", "success+2组", f"success={result['success']}, time={t1 - t0:.2f}s", result.get("success", False) and t1 - t0 < 15, f"耗时={t1 - t0:.2f}s")
    ...
    record("excel_query", "100K行COUNT(*)", "success+cnt=100000", f"success={result['success']}, time={t1 - t0:.2f}s", result.get("success", False) and t1 - t0 < 20, f"耗时={t1 - t0:.2f}s")
    ...
    record("excel_query", "100K行ORDER BY DESC LIMIT3", "success+<20s", f"success={result['success']}, time={t1 - t0:.2f}s", result.get("success", False) and t1 - t0 < 30, f"耗时={t1 - t0:.2f}s")
    ...
    record(
        "excel_query", "30列宽表SELECT*", f"success+{col_count}列", f"cols={col_count}, time={t1 - t0:.2f}s", result.get("success", False) and col_count == 30, f"耗时={t1 - t0:.2f}s, 列数={col_count}"
    )

    # A8: 30列宽表WHERE+聚合
    t0 = time.time()
    result = execute_advanced_sql_query(path_wide, "SELECT Col0, COUNT(*) as cnt, AVG(Col1) as avg1 FROM `Sheet1` GROUP BY Col0 LIMIT 5")
    t1 = time.time()

    record("excel_query", "30列宽表GROUP BY", "success", f"time={t1 - t0:.2f}s", result.get("success", False), f"耗时={t1 - t0:.2f}s")
    ...
    long_str = "A" * 20000
    # 需要通过API写入超长文本
    result = execute_advanced_update_query(path_longtext, f"UPDATE Sheet1 SET Name = '{long_str}' WHERE ID = 1")

    record("excel_update_query", "20000字符超长文本UPDATE", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])
    ...
    record("excel_delete_query", "50K行DELETE WHERE>49995", "success", f"time={t1 - t0:.2f}s", result.get("success", False), f"耗时={t1 - t0:.2f}s")
...
def test_group_b_permission_security():
    ...
    record("excel_query", "路径=/etc/shadow(敏感文件)", "应失败", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:80])
    ...
    result = execute_advanced_sql_query("../../etc/passwd", "SELECT * FROM Sheet1")
    record("excel_query", "路径遍历../../etc/passwd", "应失败/规范化", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:80])
    ...
    try:
        ...
        record("excel_query", "symlink→/etc/passwd", "N/A(symlink创建失败)", str(e), False, f"exception: {e}")

    # B6: 空路径
    result = execute_advanced_sql_query("", "SELECT * FROM Sheet1")
    record("excel_query", "空字符串路径", "应失败", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:80])

    # B7: 只有空格的路径
    result = execute_advanced_sql_query("   ", "SELECT * FROM Sheet1")
    record("excel_query", "纯空格路径", "应失败", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:80])
    ...
    record("excel_query", "LOAD DATA INFILE攻击", "应失败/不支持", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:100])
...
def test_group_c_sql_injection_new_vectors():
    ...
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1 UNION SELECT 1,2,3")
    is_union_executed = result.get("success", False) and len(result.get("data", [])) > 6
    record(
        "excel_query",
        "UNION注入 SELECT*UNION SELECT 1,2,3",
        "应拒绝或无害",
    ...
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1 UNION SELECT 'hacked'")
    record("excel_query", "UNION列数不匹配", "应失败", f"success={result['success']}", not result.get("success", False), str(result.get("message", ""))[:80])

    # C3: Stack queries with semicolon (P0-2 regression)
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1; SELECT * FROM Sheet1")
    is_multi = result.get("success", False) and ("多语句" in str(result.get("message", "")) or "multi" in str(result.get("data", "")).lower())
    record(
        "excel_query",
        "[P0-2回归]分号多语句SELECT;SELECT",
        "应拒绝!",
    ...
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = 0 WHERE ID = 1; DROP TABLE Sheet1")
    record("excel_update_query", "[P0-4回归]UPDATE分号注入", "应拒绝!", f"success={result['success']}", not result.get("success", False), f"仍可执行! affected={result.get('affected_rows', '?')}")
    ...
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = -9999 -- WHERE ID = 99999")
    ...
    all_updated = result.get("success", False) and cnt_before == cnt_after and cnt_after != "?" and int(cnt_after) > 1
    record(
        "excel_update_query",
        "[P0-7回归]注释符注入全表篡改",
        "应拒绝!",
    ...
    result = execute_advanced_insert_query(path, "INSERT INTO Sheet1 (ID, Name, Value) VALUES (999, 'hack', 0); DROP TABLE Sheet1")
    record("excel_insert_query", "[P0-5回归]INSERT分号注入", "应拒绝!", f"success={result['success']}", not result.get("success", False), f"仍可执行!")

    # C7: DELETE semicolon injection (P0-6 regression)
    result = execute_advanced_delete_query(path, "DELETE FROM Sheet1 WHERE ID = 99999; DROP TABLE Sheet1")
    record("excel_delete_query", "[P0-6回归]DELETE分号注入", "应拒绝!", f"success={result['success']}", not result.get("success", False), f"仍可执行!")

    # C8: Hex-encoded attack vector
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1 WHERE Name = 0x6861636B")  # hex for 'hack'
    record(
        "excel_query",
        "Hex编码注入 0x6861636B",
        "应失败/无匹配",
    ...
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1 WHERE Name = CHAR(104,97,99,107)")  # CHAR() for 'hack'
    record("excel_query", "CHAR()函数注入", "应失败/不支持/无匹配", f"success={result['success']}", True, str(result.get("message", ""))[:80])

    # C10: Double URL encoding / Unicode normalization
    result = execute_advanced_sql_query(path, "SELECT * FROM Sheet1 WHERE Name = 'ｈａｃｋ'")  # fullwidth characters
    record("excel_query", "Unicode全角字符注入", "应无匹配(正常)", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])
...
def test_group_d_type_boundary():
    ...
    # D1: 极大浮点数
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = 1.7976931348623157e+308 WHERE ID = 1")
    record("excel_update_query", "float MAX(double)", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # D2: 极小浮点数
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = -1.7976931348623157e+308 WHERE ID = 2")
    record("excel_update_query", "float MIN(double)负", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # D3: 超大整数
    big_int = 10**18
    result = execute_advanced_update_query(path, f"UPDATE Sheet1 SET Value = {big_int} WHERE ID = 3")
    record("excel_update_query", "超大整数10^18", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])
    ...
    if verify.get("success") and len(verify.get("data", [])) > 1:
        for row in verify["data"][1:]:
            record("excel_query", "边界值回读验证", "数值正确", f"ID={row[0]}, V={row[1]}", True, "")

    # D4: NaN值
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = CAST('nan' AS FLOAT64) WHERE ID = 1")
    nan_handled = result.get("success", False) or ("error" in str(result.get("message", "")).lower() or "invalid" in str(result.get("message", "")).lower())
    record("excel_update_query", "NaN值写入", "success或优雅错误", f"success={result['success']}", nan_handled, str(result.get("message", ""))[:80])

    # D5: Infinity值
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = CAST('inf' AS FLOAT64) WHERE ID = 1")
    inf_handled = result.get("success", False) or ("error" in str(result.get("message", "")).lower())
    record("excel_update_query", "Infinity值写入", "success或优雅错误", f"success={result['success']}", inf_handled, str(result.get("message", ""))[:80])

    # D6: 负零
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = -0.0 WHERE ID = 1")
    record("excel_update_query", "负零(-0.0)", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # D7: 精度丢失测试
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = 0.1 + 0.2 WHERE ID = 1")
    record("excel_update_query", "浮点精度0.1+0.2", "success(~0.3)", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # D8: 混合类型列写入 (int→string)
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Name = 12345 WHERE ID = 1")
    record(
        "excel_update_query",
        "int写入字符串列",
        "success或类型错误",
    ...
    # D9: 布尔值写入
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = TRUE WHERE ID = 1")
    bool_ok = result.get("success", False)
    record("excel_update_query", "布尔TRUE写入数值列", "success", f"success={bool_ok}", bool_ok, str(result.get("message", ""))[:80])
...
def test_group_e_concurrent_repeat():
    ...
    # E1: 同一单元格连续写入10次
    for i in range(10):
        result = execute_advanced_update_query(path, f"UPDATE Sheet1 SET Value = {i} WHERE ID = 1")
    ...
    record("excel_update_query", "同一单元格连续写入10次", f"最终值=9, 实际={final_val}", f"val={final_val}", final_val == 9, f"最终值={final_val}")

    # E2: 连续查询不同sheet (单sheet多次)
    for i in range(20):
        result = execute_advanced_sql_query(path, f"SELECT * FROM Sheet1 WHERE ID = {i % 5 + 1}")

    record("excel_query", "连续快速查询20次", "全部success", "all_success", result.get("success", False), f"第20次结果: success={result.get('success')}")

    # E3: INSERT相同主键多次
    results = []
    for i in range(3):
        r = execute_advanced_insert_query(path, f"INSERT INTO Sheet1 (ID, Name, Value) VALUES ({100 + i}, 'dup', {i})")
        results.append(r.get("success", False))

    all_insert_ok = all(results)
    record("excel_insert_query", "连续INSERT 3行不同ID", "全部success", f"results={results}", all_insert_ok, f"插入结果={results}")
    ...
    chain_ok = r1.get("success") and r2.get("success") and abs((val2 or 0) - (expected_val2 or 0)) < 0.01
    record("chain", "UPDATE→READ→UPDATE→READ一致性链", f"val1={val1}, val2≈{expected_val2}", f"val1={val1}, val2={val2}", chain_ok, f"一致性检查: {val1} → {val2} (期望≈{expected_val2})")
...
def test_group_f_multi_sheet():
    ...
    # F1: 跨Sheet JOIN
    result = execute_advanced_sql_query(path, "SELECT p.Name, g.GuildName, p.Level FROM Players p JOIN Guilds g ON p.GuildID = g.GuildID ORDER BY p.Level DESC LIMIT 5")
    record(
        "excel_query",
    ...
    record("excel_query", "跨Sheet聚合GROUP BY+JOIN", "success+4组", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # F3: CTE + 多Sheet
    result = execute_advanced_sql_query(path, "WITH TopPlayers AS (  SELECT * FROM Players WHERE Level >= 60) SELECT tp.Name, g.GuildName FROM TopPlayers tp JOIN Guilds g ON tp.GuildID = g.GuildID")
    record("excel_query", "CTE+跨SheetJOIN", "success", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # F4: 子查询跨Sheet
    result = execute_advanced_sql_query(path, "SELECT * FROM Players WHERE GuildID IN (SELECT GuildID FROM Guilds WHERE GuildName = 'Guild1')")
    record("excel_query", "子查询IN(跨Sheet)", "success+~5行", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])

    # F5: UPDATE跨Sheet引用 (应该失败)
    result = execute_advanced_update_query(path, "UPDATE Players SET Level = (SELECT MAX(Level) FROM Players) + 1 WHERE PlayerID = 1")
    record(
        "excel_update_query",
        "UPDATE SET=(子查询)",
        "success或优雅错误",
    ...
    # F6: 不同Sheet同名列JOIN
    # 两表都有GuildID，验证别名处理
    result = execute_advanced_sql_query(path, "SELECT p.PlayerID, p.GuildID as P_Guild, g.GuildID as G_Guild, g.GuildName FROM Players p INNER JOIN Guilds g ON p.GuildID = g.GuildID LIMIT 3")
    record("excel_query", "跨Sheet同名列别名处理", "success+区分两列", f"success={result['success']}", result.get("success", False), str(result.get("message", ""))[:80])
...
def test_group_g_p0_regression():
    ...
    record("[P0-2]", "SELECT分号多语句注入", "应拒绝❌仍存在", f"executed={is_vuln}", not is_vuln, f"🚨 仍存在! executed={is_vuln}")

    # G3: [P0-4] UPDATE 分号多语句
    result = execute_advanced_update_query(path, "UPDATE Sheet1 SET Value = 0; DROP SHEET1")
    is_vuln = result.get("success", False)
    record("[P0-4]", "UPDATE分号多语句注入", "应拒绝❌仍存在", f"executed={is_vuln}", not is_vuln, f"🚨 仍存在! executed={is_vuln}")

    # G4: [P0-5] INSERT 分号多语句
    result = execute_advanced_insert_query(path, "INSERT INTO Sheet1 VALUES(999,'x',0); DROP SHEET1")
    is_vuln = result.get("success", False)
    record("[P0-5]", "INSERT分号多语句注入", "应拒绝❌仍存在", f"executed={is_vuln}", not is_vuln, f"🚨 仍存在! executed={is_vuln}")

    # G5: [P0-6] DELETE 分号多语句
    result = execute_advanced_delete_query(path, "DELETE FROM Sheet1 WHERE 1=1; DROP SHEET1")
    is_vuln = result.get("success", False)
    record("[P0-6]", "DELETE分号多语句注入", "应拒绝❌仍存在", f"executed={is_vuln}", not is_vuln, f"🚨 仍存在! executed={is_vuln}")
    ...
    record("[P0-7]", "UPDATE注释符全表篡改", "应拒绝❌仍存在", f"before={cnt_b}, after={cnt_a}", not is_vuln, f"🚨 仍存在! 全表被改 {cnt_b}→{cnt_a}")
    ...
    is_fixed = val == 999
    record("[P0-3✅]", "uint8溢出修复 V=999", f"应为999, 实际={val}", f"val={val}", is_fixed, f"修复{'有效' if is_fixed else '失效'}! val={val}")
