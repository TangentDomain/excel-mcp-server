...
from excel_mcp_server_fastmcp.core.excel_converter import ExcelConverter
from excel_mcp_server_fastmcp.models.types import OperationResult
from excel_mcp_server_fastmcp.utils.exceptions import ExcelFileNotFoundError
...
class TestExcelConverter:
    ...
    def test_converter_init(self):
        """测试转换器初始化"""
        with patch("excel_mcp_server_fastmcp.utils.validators.ExcelValidator.validate_file_path", return_value="test.xlsx"):
            converter = ExcelConverter("test.xlsx")
            assert converter is not None
            assert converter.file_path == "test.xlsx"

    @patch("excel_mcp_server_fastmcp.core.excel_converter.load_workbook")
    ...
    def test_convert_to_csv_success(self, mock_exists, mock_load_workbook):
        ...
        # Mock工作簿和工作表
        mock_worksheet = Mock()
        mock_worksheet.iter_rows.return_value = [["Name", "Age", "City"], ["Alice", 25, "New York"], ["Bob", 30, "Los Angeles"]]
    ...
    def test_convert_to_csv_file_not_found(self, mock_exists):
        ...
        with patch("excel_mcp_server_fastmcp.utils.validators.ExcelValidator.validate_file_path", return_value="nonexistent.xlsx"):
    ...
    def test_supported_formats(self):
        ...
        # 可能还有其他格式的转换方法
        potential_methods = ["convert_to_xml", "convert_to_html", "convert_to_txt"]
    ...
    @pytest.mark.parametrize("input_format,output_format", [("xlsx", "csv"), ("xlsm", "csv"), ("xlsx", "json"), ("xlsm", "json")])
    def test_format_combinations(self, input_format, output_format):
        """测试不同格式组合的转换"""
        with patch("excel_mcp_server_fastmcp.utils.validators.ExcelValidator.validate_file_path", return_value=f"test.{input_format}"):
