...
from src.excel_mcp.core.excel_converter import ExcelConverter
from src.excel_mcp.models.types import OperationResult
from src.excel_mcp.utils.exceptions import ExcelFileNotFoundError
...
class TestExcelConverter:
    ...
    def test_converter_init(self):
        """测试转换器初始化"""
        with patch('src.excel_mcp.utils.validators.ExcelValidator.validate_file_path', return_value="test.xlsx"):
            converter = ExcelConverter("test.xlsx")
            assert converter is not None
            assert converter.file_path == "test.xlsx"

    @patch('src.excel_mcp.core.excel_converter.load_workbook')
    ...
    def test_convert_to_csv_file_not_found(self, mock_exists):
        ...
        with patch('src.excel_mcp.utils.validators.ExcelValidator.validate_file_path', return_value="nonexistent.xlsx"):
    ...
    def test_format_combinations(self, input_format, output_format):
        """测试不同格式组合的转换"""
        with patch('src.excel_mcp.utils.validators.ExcelValidator.validate_file_path', return_value=f"test.{input_format}"):
