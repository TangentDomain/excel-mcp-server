...
def _round29_setup():
    """pytest autouse fixture: ensure TEST_DIR exists before each test"""
    if TEST_DIR.exists():
...
def test_sql_injection_select():
    ...
    "多语句" in str(r.get("message", "")) or r.get("success", False)
    record(
        "A1_SELECT分号多语句",
    ...
    record("A3_注释符尾随", "SELECT * FROM Sheet1 -- 注释", "应正常执行或报错", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
    ...
    record("A8_批量分号+DROP", "SELECT 1; SELECT 2; SELECT 3; DROP TABLE xxx", "必须拒绝!", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:150])
    ...
    record("A10_反引号标识符", "SELECT * FROM `Sheet1`", "可能成功或报错", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])
...
def test_sql_injection_write():
    ...
    record("B2_INSERT分号注入", "INSERT ... ; DROP TABLE ...", "必须拒绝!", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:150])
    ...
    record("B3_DELETE分号注入", "DELETE ... ; DROP TABLE ...", "必须拒绝!", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:150])
    ...
    record("B5_INSERT单引号未转义", "INSERT ... VALUES (99,O'Connor,0,test)", "语法错误", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:100])
    ...
    record("B6_正常UPDATE对照", "UPDATE Sheet1 SET Price = 777 WHERE ID = 1", "成功更新", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
...
def test_boundary_combinations():
    ...
    record("C3_超长列名查询", "SELECT VeryLongColumnName_1_ForTestingPurpose FROM WideColumns", "成功查询", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
    ...
    record("C6_公式SheetWHERE过滤", "SELECT * FROM FormulaSheet WHERE A > 20", "过滤成功", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])

    # C7: 公式Sheet聚合
    r = execute_advanced_sql_query(f, "SELECT SUM(C) as TotalC FROM FormulaSheet")
    record("C7_公式Sheet聚合SUM(C)", "SELECT SUM(C) as TotalC FROM FormulaSheet", "聚合公式计算列", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])

    # C8: 特殊Sheet名 + UPDATE
    r = execute_advanced_update_query(f, "UPDATE `Sheet-Test_数据` SET Value = 999 WHERE ID = 1")
    record("C8_特殊Sheet名UPDATE", "UPDATE `Sheet-Test_数据` SET Value = 999 WHERE ID = 1", "更新成功", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])

    # C9: 特殊Sheet名 + DELETE
    r = execute_advanced_delete_query(f, "DELETE FROM `Sheet-Test_数据` WHERE ID = 99")
    record("C9_特殊Sheet名DELETE", "DELETE FROM `Sheet-Test_数据` WHERE ID = 99", "零匹配删除成功", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
...
def test_known_issues_regression():
    ...
    record("D2_P0-3回归_uint8溢出修复验证", "UPDATE MixedTypes SET V = 999 (混合类型Sheet, V>255)", "V应为999(非231截断)", f"V={actual_val}", is_fixed, f"actual_value={actual_val}, expected=999")
    ...
    record("D5_P2-2回归_CASE算术混合", "SELECT col * CASE WHEN...END", "已知P2: CASE不能做算术操作数", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:100])
...
def test_advanced_injection_vectors():
    ...
    # E2: 大写混淆关键字
    r = execute_advanced_sql_query(f, "sElEcT * FrOm ShEeT1")
    record("E2_大小写混淆关键字", "sElEcT * FrOm ShEeT1", "大小写不敏感(应正常工作)", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
    ...
    record("E4_十六进制字面量", "SELECT ... WHERE Name = 0x53776F7264", "可能不支持hex字面量", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])
    ...
    record("E6_标量子查询", "SELECT ... WHERE ID = (SELECT ID FROM ... LIMIT 1)", "子查询支持", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])
    ...
    record("E7_UPDATE子查询SET", "UPDATE SET col = (SELECT AVG(col) ...)", "子查询赋值", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])

    # E8: 空字符串边界
    r = execute_advanced_sql_query(f, "SELECT * FROM Sheet1 WHERE Name = ''")
    record(
        "E8_空字符串匹配", "SELECT ... WHERE Name = ''", "空字符串比较", f"success={r.get('success')}, rows={len(r.get('data', [])) - 1 if r.get('data') else 0}", True, str(r.get("message", ""))[:100]
    )

    # E9: 极端数值边界
    r = execute_advanced_sql_query(f, "SELECT * FROM Sheet1 WHERE Price > 1e308")
    record("E9_极端浮点(inf边界)", "SELECT ... WHERE Price > 1e308", "inf边界处理", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])

    # E10: 负数索引/偏移
    r = execute_advanced_sql_query(f, "SELECT * FROM Sheet1 ORDER BY ID DESC LIMIT -1")
    record("E10负数LIMIT", "SELECT ... ORDER BY ID DESC LIMIT -1", "负数LIMIT处理", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])
...
def test_encoding_attacks():
    ...
    record("F1_全角SQL关键字", "ＳＥＬＥＣＴ ＊ ＦＲＯＭ EncTest (fullwidth)", "应失败(全角不是合法SQL)", f"success={r.get('success')}", not r.get("success", False), str(r.get("message", ""))[:100])

    # F2: Tab/换行在SQL中
    r = execute_advanced_sql_query(f, "SELECT\t*\nFROM\nEncTest")
    record("F2_SQL中含Tab和换行", "SELECT\\t*\\nFROM\\nEncTest", "空白符通常被忽略", f"success={r.get('success')}", r.get("success", False), str(r.get("message", ""))[:100])

    # F3: 零宽字符
    r = execute_advanced_sql_query(f, "SELECT * FROM EncTest\u200b WHERE ID = 1")
    record("F3_零宽字符", "SELECT * FROM EncTest\\u200b WHERE ID = 1", "零宽字符处理", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])

    # F4: 方向覆盖字符(RLO)
    r = execute_advanced_sql_query(f, "SELECT * FROM EncTest\u202eWHERE ID = 1")
    record("F4_RLO方向覆盖字符", "SELECT * FROM EncTest\\u202eWHERE ID = 1", "方向覆盖字符", f"success={r.get('success')}", True, str(r.get("message", ""))[:100])
