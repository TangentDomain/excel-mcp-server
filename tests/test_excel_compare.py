...
from excel_mcp_server_fastmcp.core.excel_compare import ExcelComparer
from excel_mcp_server_fastmcp.models.types import ComparisonOptions, ComparisonResult, DifferenceType, FieldDifference, OperationResult, RowDifference
from excel_mcp_server_fastmcp.utils.exceptions import ExcelFileNotFoundError, SheetNotFoundError
...
class TestExcelComparer:
    ...
    def test_comparer_init_custom_options(self):
        """测试使用自定义选项初始化比较器"""
        options = ComparisonOptions(compare_formulas=True, compare_formats=True, ignore_empty_cells=False)
        comparer = ExcelComparer(options)
        assert comparer.options is options

    @patch("excel_mcp_server_fastmcp.core.excel_compare.load_workbook")
    @patch("excel_mcp_server_fastmcp.utils.validators.ExcelValidator.validate_file_path")
    ...
    def test_compare_files_invalid_path(self):
        ...
        with patch("excel_mcp_server_fastmcp.utils.validators.ExcelValidator.validate_file_path") as mock_validate:
    ...
    @pytest.mark.parametrize("compare_formulas,compare_formats", [(True, True), (True, False), (False, True), (False, False)])
    def test_comparison_options_variations(self, compare_formulas, compare_formats):
        """测试不同的比较选项组合"""
        options = ComparisonOptions(compare_formulas=compare_formulas, compare_formats=compare_formats)
