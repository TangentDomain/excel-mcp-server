...
class TestSQLQueryFunctions:
    ...
    def test_join_query(self, game_xlsx):
        """JOIN关联查询"""
        q = AdvancedSQLQueryEngine()
        result = q.execute_sql_query(game_xlsx, "SELECT a.技能名称, b.装备名称 FROM 技能表 a JOIN 装备表 b ON a.技能ID = b.技能ID")
        assert result["success"], f"JOIN查询失败: {result.get('message', '')}"

    def test_group_by(self, game_xlsx):
        """GROUP BY聚合查询"""
        q = AdvancedSQLQueryEngine()
        result = q.execute_sql_query(game_xlsx, "SELECT 类型, COUNT(*) as cnt, AVG(伤害) as avg_dmg FROM 技能表 GROUP BY 类型")
        assert result["success"], f"GROUP BY查询失败: {result.get('message', '')}"

    def test_subquery(self, game_xlsx):
        """子查询"""
        q = AdvancedSQLQueryEngine()
        result = q.execute_sql_query(game_xlsx, "SELECT * FROM 技能表 WHERE 伤害 > (SELECT AVG(伤害) FROM 技能表)")
        assert result["success"], f"子查询失败: {result.get('message', '')}"

    def test_order_by(self, game_xlsx):
        """ORDER BY排序"""
        q = AdvancedSQLQueryEngine()
        result = q.execute_sql_query(game_xlsx, "SELECT * FROM 技能表 ORDER BY 伤害 DESC")
