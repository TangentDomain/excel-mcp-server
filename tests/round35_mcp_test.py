...
# 测试数据准备
# ============================================================
TEST_DIR = Path(tempfile.mkdtemp(prefix="r35_test_"))
BASE_FILE = TEST_DIR / "r35_base.xlsx"
...
def setup_test_file():
    ...
    for i in range(1, 11):
        ws2.append([i, f"Equip-{i}", i * 5 + 10, round(i * 99.9, 2), ["Common", "Rare", "Epic", "Legendary"][i % 4]])
...
def test_group_a_format_compatibility():
    ...
    # A1: 标准xlsx文件基础读取
    run_test("A", "A1 标准xlsx SELECT *", lambda: check_api_read(base, "SELECT * FROM Sheet1", expect_rows=20))

    # A2: 标准xlsx WHERE过滤
    run_test("A", "A2 WHERE条件过滤", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID > 15", expect_rows=5))

    # A3: 聚合函数
    run_test("A", "A3 聚合COUNT/SUM/AVG", lambda: check_api_read(base, "SELECT COUNT(*) as cnt, SUM(Value) as total, AVG(Price) as avg_p FROM Sheet1"))

    # A4: 空文件处理
    empty_file = setup_empty_file()
    run_test("A", "A4 空文件(无数据)读取", lambda: check_api_read(empty_file, "SELECT * FROM Sheet1", expect_error=True), expected_pass=True)  # 预期会报错或返回空

    # A5: 无表头文件
    no_header = setup_no_header_file()
    run_test("A", "A5 无表头文件读取", lambda: check_api_read(no_header, "SELECT * FROM Sheet1"), expected_pass=True)  # 可能成功(用第一行作表头)也可能失败

    # A6: 单单元格文件
    single = setup_single_cell_file()
    run_test("A", "A6 单单元格文件读取", lambda: check_api_read(single, "SELECT * FROM Sheet1"), expected_pass=True)

    # A7: 宽表(30列)读取
    wide = setup_wide_table_file()
    run_test("A", "A7 宽表(30列x5行) SELECT *", lambda: check_api_read(wide, "SELECT * FROM Wide", expect_rows=5))

    # A8: 宽表指定列查询
    run_test("A", "A8 宽表指定列查询", lambda: check_api_read(wide, "SELECT Col_0, Col_29 FROM Wide", expect_rows=5))

    # A9: 特殊结构(合并单元格+多行表头)
    special = setup_special_structure_file()
    run_test("A", "A9 特殊结构(合并单元格)读取", lambda: check_api_read(special, "SELECT * FROM Special"), expected_pass=True)

    # A10: 中文Sheet名操作
    run_test("A", "A10 中文Sheet名查询", lambda: check_api_read(base, "SELECT * FROM 装备配置", expect_rows=10))

    # A11: 中文Sheet名WHERE
    run_test("A", "A11 中文Sheet名+WHERE", lambda: check_api_read(base, "SELECT * FROM 装备配置 WHERE Rarity = 'Legendary'", expect_rows=3))

    # A12: ORDER BY + LIMIT
    run_test("A", "A12 ORDER BY DESC LIMIT", lambda: check_api_read(base, "SELECT Name, Price FROM Sheet1 ORDER BY Price DESC LIMIT 3", expect_rows=3))

    # A13: GROUP BY + HAVING
    run_test("A", "A13 GROUP BY + HAVING", lambda: check_api_read(base, "SELECT Rarity, COUNT(*) as cnt, AVG(BaseAtk) as avg_atk FROM 装备配置 GROUP BY Rarity HAVING COUNT(*) >= 2"))

    # A14: INSERT到标准文件
    run_test("A", "A14 INSERT新行", lambda: check_api_insert(base, "INSERT INTO Sheet1 (ID, Name, Value, Price, Score) VALUES (999, 'New-Item', 99999, 999.99, 99900)"))

    # A15: 验证INSERT后数据
    run_test("A", "A15 INSERT后回读验证", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID = 999", expect_rows=1))

    # A16: UPDATE操作
    run_test("A", "A16 UPDATE单行", lambda: check_api_update(base, "UPDATE Sheet1 SET Value = 88888 WHERE ID = 999", expect_affected=1))

    # A17: UPDATE后验证
    run_test("A", "A17 UPDATE后值验证", lambda: check_api_read(base, "SELECT Value FROM Sheet1 WHERE ID = 999"))

    # A18: DELETE操作
    run_test("A", "A18 DELETE插入的行", lambda: check_api_delete(base, "DELETE FROM Sheet1 WHERE ID = 999", expect_affected=1))

    # A19: DELETE后验证
    run_test("A", "A19 DELETE后确认不存在", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID = 999", expect_rows=0))

    # A20: CTE子查询
    run_test("A", "A20 CTE子查询", lambda: check_api_read(base, "WITH TopItems AS (SELECT * FROM Sheet1 ORDER BY Price DESC LIMIT 5) SELECT * FROM TopItems WHERE Value > 50", expect_rows=5))

    # A21: CASE WHEN表达式
    run_test("A", "A21 CASE WHEN分类", lambda: check_api_read(base, "SELECT Name, CASE WHEN Price > 20 THEN 'Expensive' ELSE 'Cheap' END as Tier FROM Sheet1 ORDER BY Price DESC LIMIT 5"))

    # A22: LIKE模糊匹配
    run_test("A", "A22 LIKE模式匹配", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE Name LIKE 'Item-1%'"))

    # A23: IN列表
    run_test("A", "A23 IN列表过滤", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID IN (1, 3, 5, 7)", expect_rows=4))

    # A24: BETWEEN范围
    run_test("A", "A24 BETWEEN范围查询", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID BETWEEN 5 AND 10", expect_rows=6))

    # A25: 跨Sheet JOIN
    run_test("A", "A25 跨Sheet JOIN", lambda: check_api_read(base, "SELECT s.Name, s.Price, e.Rarity FROM Sheet1 s JOIN 装备配置 e ON s.ID = e.ID"))

    # A26: 窗口函数
    run_test("A", "A26 窗口函数RANK", lambda: check_api_read(base, "SELECT Name, Price, RANK() OVER (ORDER BY Price DESC) as rnk FROM Sheet1 LIMIT 10"))

    # A27: 子查询IN
    run_test("A", "A27 子查询IN", lambda: check_api_read(base, "SELECT * FROM Sheet1 WHERE ID IN (SELECT ID FROM 装备配置 WHERE Rarity = 'Epic')"))

    # A28: 批量UPDATE
    run_test("A", "A28 批量UPDATE表达式", lambda: check_api_update(base, "UPDATE Sheet1 SET Value = ROUND(Value * 1.05, 2) WHERE ID <= 5", expect_affected=5))

    # A29: CSV导入测试
    csv_file = setup_csv_for_import()
    run_test("A", "A29 CSV文件准备就绪", lambda: {"success": os.path.exists(csv_file), "path": csv_file})

    # A30: 使用excel_convert_format转换
    run_test("A", "A30 xlsx→csv格式转换", lambda: test_format_conversion(base))
...
def test_group_b_file_boundaries():
    ...
    run_test("B", "B1 文件名含空格读取", lambda: check_api_read(space_file, "SELECT * FROM Sheet1 LIMIT 3"))

    # B2: 文件名含中文
    cn_file = os.path.join(TEST_DIR, "测试文件.xlsx")
    shutil.copy(base, cn_file)
    run_test("B", "B2 文件名含中文读取", lambda: check_api_read(cn_file, "SELECT * FROM Sheet1 LIMIT 3"))

    # B3: 文件名含特殊字符(下划线/点/括号)
    special_name_file = os.path.join(TEST_DIR, "file_v2.0(test).xlsx")
    shutil.copy(base, special_name_file)
    run_test("B", "B3 文件名含特殊字符(.())", lambda: check_api_read(special_name_file, "SELECT * FROM Sheet1 LIMIT 3"))
    ...
    run_test("B", "B4 深层子目录路径", lambda: check_api_read(deep_file, "SELECT * FROM Sheet1 LIMIT 3"))

    # B5: 不存在的文件
    run_test("B", "B5 不存在的文件路径", lambda: check_api_read("/nonexistent/path/file.xlsx", "SELECT * FROM Sheet1"), expected_pass=False)  # 预期失败

    # B6: 非Excel文件(文本文件伪装)
    fake_file = os.path.join(TEST_DIR, "fake.xlsx")
    with open(fake_file, "wb") as f:
        f.write(b"This is not an Excel file at all!")
    run_test("B", "B6 非Excel文件(伪装xlsx)", lambda: check_api_read(fake_file, "SELECT * FROM Sheet1"), expected_pass=False)
    ...
    run_test("B", "B7 截断的xlsx文件", lambda: check_api_read(trunc_file, "SELECT * FROM Sheet1"), expected_pass=False)

    # B8: 空文件(0字节)
    zero_file = os.path.join(TEST_DIR, "zero_bytes.xlsx")
    with open(zero_file, "wb") as f:
        pass  # 创建0字节文件
    run_test("B", "B8 0字节空文件", lambda: check_api_read(zero_file, "SELECT * FROM Sheet1"), expected_pass=False)
...
def test_group_d_known_issues():
    ...
    # D1: P1 - 连字符Sheet名不支持
    run_test("D", "D1 [P1] 连字符Sheet名 `my-data` SELECT", lambda: check_api_read(base, "SELECT * FROM `my-data`"), expected_pass=False)  # 已知不支持
    ...
    def d2_formula_col_test():
        ...
        if result.get("success"):
            list(result.get("columns", [])) if "columns" in result else []
            data = result.get("data", [])
    ...
    def d3_float_max_test():
        ...
        # 写入float max
        execute_advanced_update_query(float_file, "UPDATE Sheet1 SET FloatVal = 1.7976931348623157e+308 WHERE ID = 1")
    ...
    run_test("D", "D4 [P2-1] UPDATE SET ||拼接", lambda: check_api_update(base, "UPDATE Sheet1 SET Name = 'X-' || Name WHERE ID = 1"), expected_pass=False)

    # D5: P2-2 CASE WHEN算术混合不支持
    run_test(
        "D", "D5 [P2-2] CASE WHEN算术混合", lambda: check_api_read(base, "SELECT Name, BaseAtk * CASE WHEN Rarity = 'Legendary' THEN 2 ELSE 1 END as Adj FROM 装备配置 LIMIT 3"), expected_pass=False
    )

    # D6: P3-1 多余逗号幽灵列
    run_test("D", "D6 [P3-1] SELECT多余逗号幽灵列", lambda: check_api_read(base, "SELECT ,,, FROM Sheet1 LIMIT 2"), expected_pass=False)  # 应该报错但实际产生幽灵列

    # D7: SQL大小写敏感性
    run_test("D", "D7 SQL大小写敏感(sheet1≠Sheet1)", lambda: check_api_read(base, "SELECT * FROM sheet1 LIMIT 1"), expected_pass=False)  # 小写sheet1应该失败
