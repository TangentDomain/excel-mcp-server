"""
集中式错误提示系统测试
验证所有非SQL错误都包含[提示]修复提示
"""

import pytest

from excel_mcp_server_fastmcp.server import _ERROR_HINTS, _fail, _infer_error_code, _wrap
...
class TestCentralizedErrorHints:
    ...
    def test_all_error_codes_have_hints(self):
        ...
        # 反向检查: 所有 hint key 都应被测试覆盖
        for code in _ERROR_HINTS:
            assert code in used_codes, f"hint key '{code}' 存在但未被测试覆盖"
...
class TestWrapHints:
    ...
    def test_wrap_existing_error_code_no_override(self):
        """已有error_code时不覆盖"""
        result = _wrap({"success": False, "message": "自定义错误", "data": None, "meta": {"error_code": "CUSTOM_CODE"}})
    ...
    def test_wrap_sql_query_info_preserved(self):
        ...
        result = _wrap({"success": False, "message": "SQL错误", "data": [], "query_info": {"error_type": "sql_syntax", "hint": "fix syntax"}})
