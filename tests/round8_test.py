...
def create_test_xlsx():
    ...
    headers_event = ["EventID", "RewardID", "RewardName", "Quantity", "Priority", "IsActive"]
    ...
    try:
        ...
        if expect_error:
            if not result['success']:
                if verify_fn(result):
                    return {"status": "PASS", "data": result.get('data', []), "message": result.get('message', '')[:200]}
                else:
                    return {"status": "FAIL", "data": None, "message": f"期望错误但验证失败: {result.get('message', '')[:200]}"}
            else:
                return {"status": "FAIL", "data": result.get('data', []), "message": "期望报错但执行成功"}
        else:
            if verify_fn(result):
                return {"status": "PASS", "data": result.get('data', []), "message": result.get('message', ''), "affected_rows": result.get('affected_rows')}
            else:
                return {"status": "FAIL", "data": result.get('data', []), "message": f"验证失败: {result.get('message', '')[:200]}"}
                
    except Exception as e:
        if tc.get("expect_error"):
            return {"status": "PASS", "data": None, "message": f"预期异常: {str(e)[:200]}"}
        return {"status": "ERROR", "data": None, "message": f"异常: {traceback.format_exc()[-500:]}"}
