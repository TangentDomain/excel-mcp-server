...
class TestWhereClauseSideEffects:
    ...
    def test_no_where_does_not_mutate_df(self, query_engine, sample_df):
        ...
        query_engine._apply_where_clause(parsed, sample_df.copy())
        # 验证原始DF未被修改
    ...
    def test_cast_where_temp_cols_cleaned(self, query_engine, sample_df):
        ...
        # 临时列必须清理干净
        assert list(df_input.columns) == original_cols, f"临时列未清理! 当前列: {list(df_input.columns)}, 原始: {original_cols}"
        assert not hasattr(df_input, "_tmp_columns"), "_tmp_columns属性未清理"
    ...
    def test_where_with_function_expr(self, query_engine, sample_df):
        ...
        assert list(df_input.columns) == original_cols, f"列被修改: {list(df_input.columns)} vs {original_cols}"
    ...
    def test_multiple_cast_conditions(self, query_engine):
        """多个CAST条件应各自创建和清理临时列"""
        df = pd.DataFrame({"a": ["10", "20", "30", "40", "50"], "b": ["1.5", "2.5", "3.5", "4.5", "5.5"], "cat": ["X", "Y", "X", "Y", "X"]})
        df_input = df.copy()
        original_cols = list(df_input.columns)

        parsed = _parse("SELECT * FROM data WHERE CAST(a AS INTEGER) > 15 AND CAST(b AS FLOAT) < 5.0")
        result = query_engine._apply_where_clause(parsed, df_input)

        # 所有临时列清理完毕
        assert list(df_input.columns) == original_cols, f"临时列残留: {[c for c in df_input.columns if c not in original_cols]}"
    ...
    def test_where_data_integrity(self, query_engine, sample_df):
        ...
        query_engine._apply_where_clause(parsed, df_input)
...
class TestWhereEdgeCases:
    ...
    def test_where_is_null(self, query_engine):
        """IS NULL WHERE不应有副作用"""
        df = pd.DataFrame({"id": [1, 2, 3], "val": ["hello", None, "world"]})
        df_input = df.copy()
