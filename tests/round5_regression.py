...
check("正常 COUNT(*)", r, r['success'] and r['data'][1][0] == 3, f"期望3, 实际{r.get('data')}")
...
check("SUM 空结果→NULL", r, r['success'] and len(r['data']) >= 2 and r['data'][1][0] is None,
      f"期望[None], 实际{r.get('data')}")

r = execute_advanced_sql_query(fp, "SELECT COUNT(*) as cnt FROM t WHERE ID > 99")
check("COUNT 空结果→0", r, r['success'] and len(r['data']) >= 2 and r['data'][1][0] == 0,
