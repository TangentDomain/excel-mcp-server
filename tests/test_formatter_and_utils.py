...
from excel_mcp_server_fastmcp.models.types import CellInfo, OperationResult, RangeInfo, RangeType, SheetInfo
from excel_mcp_server_fastmcp.utils.formatter import format_operation_result
...
class TestFormatter:
    ...
    def test_format_operation_result_success(self):
        """测试成功结果的格式化"""
        result = OperationResult(success=True, data=[["A", "B"], ["C", "D"]], metadata={"rows": 2, "columns": 2})
    ...
    def test_format_with_nested_data(self):
        """测试嵌套数据格式化"""
        result = OperationResult(success=True, data=[[{"key": "value"}], ["test"]])
    ...
    def test_format_with_none_values(self):
        """测试包含None值的格式化"""
        result = OperationResult(success=True, data=[[None, "value"], ["test", None]])
