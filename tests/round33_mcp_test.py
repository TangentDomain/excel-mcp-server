...
p0_tests = [
    ...
    ("C4: P0-6 DELETE分号多语句", "delete", "DELETE FROM Sheet1 WHERE ID = 99999; DROP TABLE Sheet1", lambda: None, lambda r: r.get("success", False), False),
    (
        "C5: P0-7 UPDATE注释符全表篡改",
        "update",
...
try:
    result = execute_advanced_sql_query(TEST_FILE, "SELECT Name, Value * CASE WHEN Value > 50 THEN 2 ELSE 1 END as adj FROM Sheet1 LIMIT 5")
    if result.get("success"):
...
# E15: 深度嵌套括号
try:
    result = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM Sheet1 WHERE (((((ID = 1))))) AND ((((Value > 0))))")
    if result.get("success"):
