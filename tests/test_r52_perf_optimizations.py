...
class TestP3Perf02_HotPathImports:
    ...
    def test_serialize_value_datetime_handling(self, sample_xlsx):
        ...
        for val, expected in test_cases:
            ...
            if expected is None:
                ...
                assert result == expected or (isinstance(result, float) and abs(result - expected) < 1e-9), f"_serialize_value({val}) = {result}, expected {expected}"
            else:
    ...
    def test_top_level_imports_present(self):
        ...
        with open(advanced_sql_query.__file__, encoding="utf-8") as f:
            source = f.read()
    ...
    def test_full_query_with_optimized_imports(self, sample_xlsx):
        ...
        r2 = execute_advanced_sql_query(sample_xlsx, "SELECT COUNT(*) as cnt, AVG(Value) as avg_val, SUM(Score) as total FROM TestData")
        assert r2["success"]
        # 聚合结果格式：第一行是表头，第二行是数据
        assert len(r2["data"]) >= 2  # 至少表头+数据行

        # WHERE 查询
        r3 = execute_advanced_sql_query(sample_xlsx, "SELECT * FROM TestData WHERE Score > 70 ORDER BY Value DESC")
        assert r3["success"]
    ...
    def test_performance_no_regression(self, sample_xlsx):
        ...
        start = time.time()
        for _ in range(10):
            execute_advanced_sql_query(sample_xlsx, "SELECT ID, Name, ROUND(Value, 1) as RndVal, Score FROM TestData WHERE Score > 50")
