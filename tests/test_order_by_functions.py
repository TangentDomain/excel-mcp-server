...
class TestOrderByComplexExpressions:
    ...
    def test_order_by_case_when(self, engine):
        ...
        result = engine.execute_sql_query(GAME_CONFIG, "SELECT skill_name, damage FROM 技能配置 ORDER BY CASE WHEN damage > 100 THEN 1 ELSE 0 END DESC")
        assert result["success"] is True
        # 高伤害在前
    ...
    def test_order_by_math_expression(self, engine):
        """ORDER BY col1 * col2"""
        result = engine.execute_sql_query(GAME_CONFIG, "SELECT skill_name, damage, cooldown FROM 技能配置 ORDER BY damage / cooldown DESC")
        assert result["success"] is True

    def test_order_by_function_with_where(self, engine):
        """ORDER BY function + WHERE 组合"""
        result = engine.execute_sql_query(GAME_CONFIG, "SELECT skill_name FROM 技能配置 WHERE skill_type = '法师' ORDER BY LENGTH(skill_name)")
        assert result["success"] is True
        # 结果数应>0且全部按长度递增
        names = [row[0] for row in result["data"][1:]]
        assert len(names) > 0
