...
class TestBoundaryValues:
    ...
    def test_float_precision_0_1(self, engine, boundary_test_file):
        """测试0.1秒精度不丢失"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 冷却时间 FROM 边界值测试 WHERE 冷却时间 = 0.1")
    ...
    def test_float_precision_0_01(self, engine, boundary_test_file):
        """测试0.01秒精度"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 冷却时间 FROM 边界值测试 WHERE 冷却时间 = 0.01")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_float_precision_0_001(self, engine, boundary_test_file):
        """测试0.001秒精度"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 冷却时间 FROM 边界值测试 WHERE 冷却时间 = 0.001")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_large_value_99999(self, engine, boundary_test_file):
        """测试99999大数值"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 价格 >= 99999")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_large_value_999999(self, engine, boundary_test_file):
        """测试999999超大数值"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 伤害值 FROM 边界值测试 WHERE 伤害值 >= 999999")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        assert len(result.get("data", [])) > 1, "应该找到超大伤害值的记录"

    def test_max_value_comparison(self, engine, boundary_test_file):
        """测试最大值比较"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 价格 = 9999999")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_zero_value(self, engine, boundary_test_file):
        """测试零值处理"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格, 冷却时间 FROM 边界值测试 WHERE 价格 = 0 AND 冷却时间 = 0")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_negative_value(self, engine, boundary_test_file):
        """测试负值处理"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 价格 < 0")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_null_handling_is_null(self, engine, boundary_test_file):
        """测试IS NULL条件 - 验证查询不崩溃"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT * FROM 边界值测试 WHERE 名称 IS NULL")
    ...
    def test_null_handling_is_not_null(self, engine, boundary_test_file):
        """测试IS NOT NULL条件"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 名称 IS NOT NULL AND 价格 IS NOT NULL")
        assert result["success"], f"查询失败: {result.get('message', '')}"
    ...
    def test_between_precision(self, engine, boundary_test_file):
        """测试BETWEEN精度"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 冷却时间 FROM 边界值测试 WHERE 冷却时间 BETWEEN 0.09 AND 0.11")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
    ...
    def test_between_large_values(self, engine, boundary_test_file):
        """测试大数值BETWEEN"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 价格 BETWEEN 99999 AND 1000000")
        assert result["success"], f"查询失败: {result.get('message', '')}"
    ...
    def test_max_with_boundary(self, engine, boundary_test_file):
        """测试MAX函数在大数值边界"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT MAX(价格) as 最高价格 FROM 边界值测试 WHERE 价格 IS NOT NULL")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        assert len(result.get("data", [])) > 1, "应该返回聚合结果"

    def test_min_with_boundary(self, engine, boundary_test_file):
        """测试MIN函数在负值边界"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT MIN(价格) as 最低价格 FROM 边界值测试 WHERE 价格 IS NOT NULL")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        assert len(result.get("data", [])) > 1, "应该返回聚合结果"

    def test_count_with_null(self, engine, boundary_test_file):
        """测试COUNT函数对NULL的处理"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT COUNT(*) as 总数 FROM 边界值测试")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        assert len(result.get("data", [])) > 1, "应该返回聚合结果"
    ...
    def test_tiny_value(self, engine, boundary_test_file):
        """测试极小值 (0.00001)"""
        result = engine.execute_sql_query(boundary_test_file, "SELECT 名称, 价格 FROM 边界值测试 WHERE 价格 <= 0.0001 AND 价格 > 0")
        assert result["success"], f"查询失败: {result.get('message', '')}"
        names = _get_column_data(result, "名称")
