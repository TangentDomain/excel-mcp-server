...
def test_group_a_concurrency():
    ...
    all_ok = all(outcomes)
    record("A1-多线程并发读取(8线程)", "A", all_ok, f"8线程同时读取, 全部成功={all_ok}, 耗时{elapsed:.2f}s", f"SELECT * FROM Sheet1 (8 threads)", "全部成功", f"{sum(outcomes)}/8 成功")
    ...
    start = time.time()
    for i in range(20):
        result = execute_advanced_update_query(f2, f"UPDATE Sheet1 SET Value = {i * 100} WHERE ID = 1")
    ...
    record(
        ...
        "20x UPDATE Sheet1 SET Value=? WHERE ID=1",
        "0错误",
    ...
    record("A3-连续写入后值正确性", "A", final_val == 1900, f"最终Value应为1900(19*100), 实际={final_val}", "最终值=1900", str(final_val))
    ...
    record("A4-交替读写(10轮)", "A", len(alt_errors) == 0, f"10轮交替读写, 错误={len(alt_errors)}", "0错误", f"{len(alt_errors)}错误")
    ...
    for i in range(10):
        ...
        if not r1.get("success"):
            sheet_errors.append(f"s1w-{i}")
        if not r2.get("success"):
            sheet_errors.append(f"eqw-{i}")
        if not r3.get("success"):
            sheet_errors.append(f"tr-{i}")
    record("A5-多Sheet混合操作(10轮)", "A", len(sheet_errors) == 0, f"3个Sheet混合操作, 错误={len(sheet_errors)}", "0错误", f"{len(sheet_errors)}错误")

    # A6: 大批量INSERT后立即查询
    f5 = copy_test_file(base, "bulk_insert_read.xlsx")
    insert_results = []
    for i in range(15):
        ir = execute_advanced_insert_query(f5, f"INSERT INTO Sheet1 (ID, Name, Value, Price, Score) VALUES ({100 + i}, 'Bulk-{i}', {i}, {i * 1.1}, {i * 10})")
    ...
    record(
        "A6-批量INSERT+立即查询",
        "A",
        all_inserts_ok and count_val == 35,
        f"15条INSERT全部成功={all_inserts_ok}, 总行数={count_val}(期望35)",
        "15 INSERT OK + COUNT=35",
        f"INSERT:{sum(insert_results)}/15, COUNT:{count_val}",
    )

    # A7: DELETE后再INSERT同ID
    f6 = copy_test_file(base, "delete_reinsert.xlsx")
    execute_advanced_delete_query(f6, "DELETE FROM Sheet1 WHERE ID = 5")
    execute_advanced_insert_query(f6, "INSERT INTO Sheet1 (ID, Name, Value, Price, Score) VALUES (5, 'Reborn', 999, 99.99, 500)")
    ...
    def write_different_row(fid, row_id):
        try:
            result = execute_advanced_update_query(fid, f"UPDATE Sheet1 SET Value = {row_id * 111} WHERE ID = {row_id}")
            with write_lock:
                if not result.get("success"):
    ...
    record("A8-并发写不同行(5线程x10行)", "A", len(write_errors) == 0, f"5线程并发写10行, 写错误={len(write_errors)}, 耗时{elapsed:.3f}s", "0写错误", f"{len(write_errors)}错误")
    ...
    record(
        ...
        "<3错误(允许偶发)",
        f"{len(stress_errors)}错误, 样例: {stress_errors[:2]}",
    )
    ...
    record(
        ...
        f"COUNT不变({before_cnt})",
        f"before={before_cnt}, after={after_cnt}, ops={ops_ok}",
    )
...
def test_group_b_p0_regression():
    ...
    record(
        "P0-2-R9 SELECT分号多语句注入",
        "B",
        not is_vuln,
        f"success={is_vuln}, 返回数据行数={data_len}",
        "应拒绝分号/返回error",
        f"success={is_vuln}, rows={data_len}",
    ...
    r = execute_advanced_insert_query(f_p05, "INSERT INTO Sheet1 (ID, Name, Value, Price, Score) VALUES (999, 'Hacked', 0, 0, 0); DROP TABLE Sheet1")
    is_vuln = r.get("success", False)
    record("P0-5-R9 INSERT分号多语句注入", "B", not is_vuln, f"success={is_vuln}, 插入了恶意数据", "应拒绝含分号的INSERT", f"success={is_vuln}", "🚨 仍存在!" if is_vuln else "✅ 已修复!")
    ...
    record(
        "P0-6-R9 DELETE分号多语句注入",
        "B",
        not is_vuln or (is_vuln and no_match),
        f"success={is_vuln}, message={msg[:60]}, 无匹配={no_match}",
        "应拒绝分号或至少报错",
        f"success={is_vuln}, no_match={no_match}",
...
def test_group_c_known_issues():
    ...
    record(
        "C1-P1 连字符Sheet名(my-data)",
        "C",
        c1_success or c1_success2,
        f"反引号版: success={c1_success}, msg={c1_msg}; 无引号: success={c1_success2}, msg={c1_msg2}",
        "支持连字符Sheet名",
        f"均失败" if (not c1_success and not c1_success2) else "部分成功",
    )
    ...
    record("C2-P2 公式列D名保留", "C", has_d, f"可用列={cols}, 含D={has_d}, 含_ROW_NUMBER_={has_row_num}", "列名包含'D'", f"列={cols}")
    ...
    if read_back is not None:
        try:
            ...
            pass
    record("C3-P2 浮点精度(π)", "C", precision_ok, f"写入π={pi_val}, 读回={read_back}", "精度损失<0.01", f"差值={abs(float(read_back) - pi_val) if read_back else 'N/A'}")
    ...
    record("C4-P2-1 UPDATE SET ||拼接", "C", c4_success, f"success={c4_success}, msg={c4_msg}", "支持||拼接", f"不支持: {c4_msg[:50]}" if not c4_success else "支持")
...
def test_group_d_error_quality():
    ...
    for name, sql, expect_success, expected_keywords in error_tests:
        ...
        try:
            ...
            record(name, "D", passed, detail, sql[:50], "期望fail+有意义错误" if not expect_success else "期望success", f"success={success}, msg={message[:40]}")

        except Exception as e:
            record(name, "D", not expect_success, f"异常: {str(e)[:60]}", sql[:50], "期望异常/错误", str(e)[:40])
...
def test_group_e_atomicity():
    ...
    execute_advanced_update_query(f_e1, "UPDATE Sheet1 SET Value = 88888 WHERE ID = 1")
    ...
    record(
        ...
        "只改Value, 其他列不变",
        f"Name:{before_data[0] if before_data else '?'}->{after_data[0] if after_data else '?'}",
    )
    ...
    ir = execute_advanced_insert_query(f_e2, "INSERT INTO Sheet1 (ID, Name, Value, Price, Score) VALUES (9999, 'TestAtomic', 1, 1, 1)")
    ...
    record(
        "E2-INSERT无副作用",
        "E",
        original_ok and cnt_increased and ir.get("success"),
        f"原ID=1: Name={old_name}(期望Item-1), Val={old_val}(期望10); 行数:{before_cnt}->{after_cnt}",
        "原数据不变, 行数+1",
        f"original_ok={original_ok}, cnt+1={cnt_increased}",
    )
    ...
    execute_advanced_delete_query(f_e3, "DELETE FROM Sheet1 WHERE ID = 15")
    ...
    record(
        "E3-DELETE精确性",
        "E",
        id1_exists and not id15_exists and (after_dc == before_dc - 1),
        f"ID=1仍在={id1_exists}, ID=15已删={not id15_exists}, 行数:{before_dc}->{after_dc}",
        "只删ID=15, 其他在",
        f"id1={id1_exists}, id15={id15_exists}, cnt:{before_dc}->{after_dc}",
    )
    ...
    record(
        "E4-WHERE无匹配UPDATE无影响",
        "E",
        unchanged and nr.get("success"),
        f"SUM(Value): {before_s}->{after_s}, unchanged={unchanged}",
        "SUM不变(无匹配不修改)",
        f"before_sum={before_s}, after_sum={after_s}",
    )

    # E5: 复杂表达式的计算正确性
    f_e5 = copy_test_file(base, "e5_complex_expr.xlsx")
    execute_advanced_update_query(f_e5, "UPDATE Sheet1 SET Value = ID * 10 + Score / 5 WHERE ID <= 5")
    ...
    record("E5-复杂表达式计算正确性", "E", calc_correct, f"Value = ID*10 + Score/5 对ID<=5的计算验证", "每行计算精确匹配", "计算正确" if calc_correct else "存在偏差")
