...
class TestInEdgeCases:
    ...
    def test_in_workaround_for_negative(self, edge_xlsx):
        """IN 负数的变通方法：用 OR 替代"""
        result = execute_advanced_sql_query(edge_xlsx, "SELECT * FROM data WHERE val_int = 0 OR val_int = 1 OR val_int = -1")
        assert result["success"] is True
        # val_int IN (0,1,-1): id=1,2,3,5 → 4行 + header
        assert len(result["data"]) >= 5
...
class TestBetweenEdgeCases:
    ...
    def test_between_inclusive(self, edge_xlsx):
        ...
        result = execute_advanced_sql_query(edge_xlsx, "SELECT * FROM data WHERE val_int BETWEEN 0 AND 10")
        assert result["success"] is True
    ...
    def test_between_reversed_range(self, edge_xlsx):
        ...
        result = execute_advanced_sql_query(edge_xlsx, "SELECT * FROM data WHERE val_int BETWEEN 10 AND 0")
        assert result["success"] is True
...
class TestOrderByEdgeCases:
    ...
    def test_order_by_nulls(self, edge_xlsx):
        """ORDER BY 含 NULL 列"""
        result = execute_advanced_sql_query(edge_xlsx, "SELECT id FROM data ORDER BY val_mixed LIMIT 5")
        assert result["success"] is True
        assert len(result["data"]) >= 6

    def test_order_by_multiple_columns(self, edge_xlsx):
        """多列 ORDER BY"""
        result = execute_advanced_sql_query(edge_xlsx, "SELECT * FROM data ORDER BY cat, val_int DESC")
        assert result["success"] is True
        assert len(result["data"]) >= 9
