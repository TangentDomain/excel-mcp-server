    ...
    for name, sql, should_succeed in tests:
        ...
        try:
                ...
                if ok:
                    ...
                    if "P0-3" in name or "极大值" in name:
                        ...
                        if vr.get("success"):
                            ...
                            if val_str == "999":
                                record(name, sql, "值=999", "✅ 未截断", True)
                                passed += 1
                            else:
            ...
            if "🔥" in sql or "Emoji" in name:
                record(name, sql, "成功", f"预期异常: {str(e)[:40]}", True)
                passed += 1
            elif should_succeed:
                record(name, sql, "成功", f"异常: {str(e)[:80]}", False, str(e)[:100])
            else:
                record(name, sql, "应拒绝", f"被拦截: {str(e)[:40]}", True)
                passed += 1
        ...
        try:
            ...
            if should_succeed:
                if ok:
                    if "P0-3" in name:
                        ...
                        if vr.get("success"):
                            ...
                            if vs == "999":
                                record(name, sql, "值=999", "✅ P0-3仍修复", True)
                                passed += 1
                            else:
                ...
                if ok:
                    ...
                    record(name, sql, "应拒绝(安全)", "✅ 正确拦截", True)
                    passed += 1
        except Exception as e:
            if not should_succeed:
                record(name, sql, "应拒绝", f"被拦截(异常): {str(e)[:50]}", True)
                passed += 1
            else:
                if any(x in name for x in ["P1-", "P2-"]):
...
def run_integrity_tests(file_path):
    ...
    for name, sql in tests:
        try:
            r = execute_advanced_sql_query(file_path, sql)
            if r.get("success"):
                record(name, sql, "数据完整", f"data={str(r.get('data', ''))[:60]}", True)
                passed += 1
            else:
...
def main():
    ...
    tp, tt = 0, 0
    p, t = run_boundary_tests(f)
    tp += p
    tt += t
    p, t = run_regression_tests(f)
    tp += p
    tt += t
    p, t = run_integrity_tests(f)
    tp += p
    tt += t
