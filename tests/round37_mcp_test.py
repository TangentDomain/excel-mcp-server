...
def test_group_b():
    ...
    result("P0-2-R10 SELECT分号注入", not is_vuln, f"success={r.get('success')}, rows={len(r.get('data', [])) if r.get('data') else 0}")
    ...
    result("P0-4-R10 UPDATE分号注入", not is_vuln, f"success={r.get('success')}, msg={str(r.get('message', ''))[:60]}")
    ...
    is_vuln = r.get("success", False)
    result("P0-5-R10 INSERT分号注入", not is_vuln, f"success={r.get('success')}, 插入恶意数据={is_vuln}")
    ...
    result("P0-6-R10 DELETE分号注入", not accepted_semicolon, f"success={accepted_semicolon}, msg={str(r.get('message', ''))[:60]}")
    ...
    result("P0-7-R10 UPDATE注释符篡改", not (r.get("success") and all_changed), f"success={r.get('success')}, sum:{before_sum}->{after_sum}, 全表篡改={all_changed}")
    ...
    result("P0-3-R10 uint8溢出修复", read_val == 999, f"写入999, 读回={read_val}")
...
def test_group_c():
    ...
    try:
        ...
        if r.get("success"):
            ...
            result("C1-跨Sheet三表JOIN", row_count > 0, f"success=True, 返回{row_count}行数据, data[1]={r.get('data', [[]])[1][:3] if len(r.get('data', [])) > 1 else 'N/A'}")
        else:
    ...
    try:
        ...
        if r.get("success"):
            ...
            result("C1b-同Sheet自JOIN", row_count > 0, f"success=True, 返回{row_count}行")
        else:
    ...
    try:
        ...
        if r.get("success"):
            row_count = len(r.get("data", [])) - 1
            result("C2-新建Sheet后立即查询", row_count == 2, f"success=True, 返回{row_count}行(期望2)")
        else:
    ...
    try:
        ...
        result("C3-重命名Sheet后查询", old_fail and new_ok, f"旧名失败={old_fail}, 新名成功={new_ok}, msg_old={str(r_old.get('message', ''))[:50]}, msg_new={str(r_new.get('message', ''))[:50]}")
    except Exception as e:
    ...
    try:
        ...
        result("C4-多Sheet同时更新", both_ok, f"update1={r1.get('success')}, update2={r2.get('success')}, price_upd={price_updated}, hp_upd={hp_updated}")
    except Exception as e:
    ...
    try:
        ...
        # 先查JOIN结果
        r_before = execute_advanced_sql_query(
            f_ref,
            """
        ...
        # 再查JOIN结果
        r_after = execute_advanced_sql_query(
            f_ref,
            """
            SELECT e.Name, m.Name as Monster
            FROM 装备 e
        ...
        result("C5-跨Sheet引用变更传播", changed, f"before={before_data}, after={after_data}, 变更传播={changed}")
    except Exception as e:
    ...
    try:
        ...
        result("C6-Sheet不存在错误处理", all_rejected, f"query_err={err1}, update_err={err2}, delete_err={err3}, insert_err={err4}, 有用消息={useful_msg}")
    except Exception as e:
    ...
    try:
        ...
        # 特殊Sheet名参与JOIN
        if cn_ok:
            r_join = execute_advanced_sql_query(
                f_special,
                """
                SELECT a.Name, b.名称
                FROM 装备 a
    ...
    try:
        ...
        result("C8-多Sheet事务隔离", tx_ok, f"update1_ok={r1.get('success')}, update2_fail={not r2.get('success')}, 装备表price正确={price_ok}")
    except Exception as e:
...
def test_group_d():
    ...
    def thread_write(thread_id):
        try:
            for i in range(5):
                r = execute_advanced_update_query(f_conc, f"UPDATE Sheet1 SET Value = {thread_id * 1000 + i} WHERE ID = {(thread_id % 20) + 1}")
    ...
    result("D1-P1 并发多线程写入(3线程x5次)", conc_ok, f"错误数={len(errors)}, 耗时={elapsed:.3f}s" + (f", 首错={errors[0]}" if errors else ""))
    ...
    try:
        ...
        result("D2-P2 公式列D名保留", has_d, f"可用列={cols}, 含D={has_d}")
    except Exception as e:
    ...
    result("D3-P2-1 ||字符串拼接", cat_ok, f"success={r.get('success')}, msg={str(r.get('message', ''))[:80]}")
    ...
    try:
        ...
        result("D4-P2-4 极端浮点值(float_max)", not file_damaged, f"write={write_ok}, read={read_ok}, val={read_val}, 文件损坏={file_damaged}")
    except Exception as e:
    ...
    if read_back is not None:
        try:
            ...
            pass
    result("D5-P2 浮点精度(π)", not precision_loss, f"写入π={pi_val}, 读回={read_back}, 精度损失={'是⚠️' if precision_loss else '否✅'}")
