...
from excel_mcp_server_fastmcp.calibrator import cmd_import, get_db_path
...
def compare_results(excel_headers, excel_rows, sqlite_headers, sqlite_rows, tol=0.01, order_by=None):
    """
...
def run_cross_validation(xlsx_path, db_path, sql, col_map, tol=0.01, order_by=None):
    """
...
class TestB1_JoinQueries:
    ...
    def test_b1_inner_join_two_tables(self, join_test_db):
        ...
        match, detail, eh, er, sh, sr = run_cross_validation(xlsx_path, db_path, sql, JOIN_TEST_COLS, order_by="skill_name")
...
class TestB2_GroupByHaving:
    ...
    def test_b2_group_by_with_count_avg(self, game_config_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, GAME_CONFIG_COLS, order_by=0, tol=0.1)
    ...
    def test_b2_having_filter(self, game_config_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, GAME_CONFIG_COLS, tol=0.1)
...
class TestB3_WindowFunctions:
    ...
    def test_b3_row_number(self, large_skills_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, LARGE_SKILLS_COLS, tol=0.01)
    ...
    def test_b3_rank_function(self, large_skills_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql_safe, LARGE_SKILLS_COLS, tol=0.01)
...
class TestB4_Subqueries:
    ...
    def test_b4_in_subquery(self, game_config_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, GAME_CONFIG_COLS, order_by=2)
    ...
    def test_b4_scalar_subquery(self, game_config_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, GAME_CONFIG_COLS, order_by=1, tol=0.1)
...
class TestB5_CaseWhen:
    ...
    def test_b5_simple_case(self, game_config_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, GAME_CONFIG_COLS, order_by=1)
    ...
    def test_b5_case_with_aggregation(self, large_skills_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, LARGE_SKILLS_COLS, order_by=1, tol=0.1)
...
class TestB6_DualHeaderQueries:
    ...
    def test_b6_dual_header_select(self, join_test_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, JOIN_TEST_COLS, order_by=2)
    ...
    def test_b6_dual_header_agg(self, join_test_db):
        ...
        match, detail, _, _, _, _ = run_cross_validation(xlsx_path, db_path, sql, JOIN_TEST_COLS, order_by=2, tol=0.1)
