...
class TestRowNumber:
    ...
    def test_row_number_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, ROW_NUMBER() OVER (ORDER BY damage DESC) as rn FROM 技能配置")
    ...
    def test_row_number_partition(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, skill_type, damage, ROW_NUMBER() OVER (PARTITION BY skill_type ORDER BY damage DESC) as rn FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
    ...
    def test_row_number_with_limit(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, ROW_NUMBER() OVER (ORDER BY damage DESC) as rn FROM 技能配置 LIMIT 5")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 5
    ...
    def test_row_number_order_by_asc(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, cooldown, ROW_NUMBER() OVER (ORDER BY cooldown ASC) as rn FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
...
class TestRank:
    ...
    def test_rank_basic(self, game_config_with_dup):
        ...
        result = execute_advanced_sql_query(game_config_with_dup, "SELECT skill_name, damage, RANK() OVER (ORDER BY damage DESC) as r FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 10
    ...
    def test_rank_partition(self, game_config_with_dup):
        ...
        result = execute_advanced_sql_query(game_config_with_dup, "SELECT skill_name, skill_type, damage, RANK() OVER (PARTITION BY skill_type ORDER BY damage DESC) as r FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
...
class TestDenseRank:
    ...
    def test_dense_rank_basic(self, game_config_with_dup):
        ...
        result = execute_advanced_sql_query(game_config_with_dup, "SELECT skill_name, damage, DENSE_RANK() OVER (ORDER BY damage DESC) as dr FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
...
class TestWindowEdgeCases:
    ...
    def test_window_without_order_by(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, ROW_NUMBER() OVER () as rn FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
    ...
    def test_window_with_where(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, ROW_NUMBER() OVER (ORDER BY damage DESC) as rn FROM 技能配置 WHERE skill_type = 'mage'")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 只应该有法师的4个技能
        assert len(rows) == 4
    ...
    def test_window_with_group_by(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_type, AVG(damage) as avg_dmg, RANK() OVER (ORDER BY AVG(damage) DESC) as r FROM 技能配置 GROUP BY skill_type")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 3个职业 + TOTAL行
        assert len(rows) >= 3
    ...
    def test_ntile_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, NTILE(3) OVER (ORDER BY damage DESC) as bucket FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
    ...
    def test_nth_value(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, NTH_VALUE(damage, 2) OVER (ORDER BY damage DESC) as val FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
    ...
    def test_window_column_not_exists(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, ROW_NUMBER() OVER (ORDER BY nonexistent DESC) as rn FROM 技能配置")
    ...
    def test_rank_dense_rank_comparison(self, game_config_with_dup):
        ...
        result = execute_advanced_sql_query(game_config_with_dup, "SELECT skill_name, damage, RANK() OVER (ORDER BY damage DESC) as r, DENSE_RANK() OVER (ORDER BY damage DESC) as dr FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
...
class TestLag:
    ...
    def test_lag_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAG(damage, 1) OVER (ORDER BY damage DESC) as prev_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
        # 第一行LAG应该是NULL（没有前一行）
    ...
    def test_lag_partition(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, skill_type, damage, LAG(damage, 1) OVER (PARTITION BY skill_type ORDER BY damage DESC) as prev_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
    ...
    def test_lag_offset_2(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAG(damage, 2) OVER (ORDER BY damage DESC) as prev2_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 前两行LAG应该是NULL
    ...
    def test_lag_default_value(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAG(damage, 1, 0) OVER (ORDER BY damage DESC) as prev_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 第一行LAG应该使用默认值0
    ...
    def test_lag_no_order_by_error(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAG(damage, 1) OVER () as prev_damage FROM 技能配置")
        assert result["success"] is False
...
class TestLead:
    ...
    def test_lead_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LEAD(damage, 1) OVER (ORDER BY damage DESC) as next_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
    ...
    def test_lead_partition(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, skill_type, damage, LEAD(damage, 1) OVER (PARTITION BY skill_type ORDER BY damage DESC) as next_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
    ...
    def test_lead_offset_2(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LEAD(damage, 2) OVER (ORDER BY damage DESC) as next2_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 最后两行LEAD应该是NULL
    ...
    def test_lead_default_value(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LEAD(damage, 1, 0) OVER (ORDER BY damage DESC) as next_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 最后一行LEAD应该使用默认值0
    ...
    def test_lead_no_order_by_error(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LEAD(damage, 1) OVER () as next_damage FROM 技能配置")
        assert result["success"] is False
        assert "需要 ORDER BY" in result["message"]
...
class TestFirstValue:
    ...
    def test_first_value_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, FIRST_VALUE(damage) OVER (ORDER BY damage DESC) as first_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
        # 所有行都应该返回第一行的伤害值(250)
    ...
    def test_first_value_partition(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, skill_type, damage, FIRST_VALUE(damage) OVER (PARTITION BY skill_type ORDER BY damage DESC) as first_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
    ...
    def test_first_value_order_by_asc(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, FIRST_VALUE(damage) OVER (ORDER BY damage ASC) as first_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        # 所有行都应该返回第一行的伤害值(0)
    ...
    def test_first_value_no_order_by_error(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, FIRST_VALUE(damage) OVER () as first_damage FROM 技能配置")
        assert result["success"] is False
        assert "需要 ORDER BY" in result["message"]
...
class TestLastValue:
    ...
    def test_last_value_basic(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAST_VALUE(damage) OVER (ORDER BY damage DESC) as last_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 8
        # 所有行都应该返回最后一行的伤害值(0)
    ...
    def test_last_value_partition(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, skill_type, damage, LAST_VALUE(damage) OVER (PARTITION BY skill_type ORDER BY damage DESC) as last_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
    ...
    def test_last_value_order_by_asc(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAST_VALUE(damage) OVER (ORDER BY damage ASC) as last_damage FROM 技能配置")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
    ...
    def test_last_value_no_order_by_error(self, game_config):
        ...
        result = execute_advanced_sql_query(game_config, "SELECT skill_name, damage, LAST_VALUE(damage) OVER () as last_damage FROM 技能配置")
        assert result["success"] is False
        assert "需要 ORDER BY" in result["message"]
