...
class TestGroupConcat:
    ...
    def test_group_concat_basic(self, dept_skills):
        ...
        result = execute_advanced_sql_query(dept_skills, "SELECT 部门, GROUP_CONCAT(技能名称) as 技能列表 FROM 技能配置 GROUP BY 部门")
    ...
    def test_group_concat_with_separator(self, dept_skills):
        ...
        result = execute_advanced_sql_query(dept_skills, "SELECT 部门, GROUP_CONCAT(skill_name, '|') as skills FROM 技能配置 GROUP BY 部门")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 3
    ...
    def test_group_concat_with_count(self, dept_skills):
        ...
        result = execute_advanced_sql_query(dept_skills, "SELECT 部门, COUNT(*) as cnt, GROUP_CONCAT(技能名称) as skills FROM 技能配置 GROUP BY 部门")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 3
    ...
    def test_group_concat_auto_alias(self, dept_skills):
        ...
        result = execute_advanced_sql_query(dept_skills, "SELECT 部门, GROUP_CONCAT(skill_name) FROM 技能配置 GROUP BY 部门")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
        assert len(rows) == 3
    ...
    def test_group_concat_having(self, dept_skills):
        ...
        result = execute_advanced_sql_query(dept_skills, "SELECT 部门, GROUP_CONCAT(技能名称) as skills FROM 技能配置 GROUP BY 部门 HAVING COUNT(*) >= 2")
        assert result["success"] is True, f"Query failed: {result.get('message')}"
        rows = _get_rows(result)
