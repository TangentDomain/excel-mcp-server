...
def join_fixtures(tmp_path):
    ...
    skills = pd.DataFrame({"skill_id": [1, 2, 3, 4], "skill_name": ["火球术", "斩击", "治疗术", "冰冻术"], "type": ["法术", "物理", "法术", "法术"], "damage": [200, 150, 0, 180]})

    unlocks = pd.DataFrame({"skill_id": [1, 2, 5], "level_req": [5, 1, 10], "cost": [100, 50, 200]})
...
class TestRightJoin:
    ...
    def test_basic_right_join(self, engine, join_fixtures):
        """基本RIGHT JOIN — 右表全部保留"""
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_id, a.skill_name, b.level_req, b.cost FROM 技能表 a RIGHT JOIN 解锁表 b ON a.skill_id = b.skill_id")
    ...
    def test_right_join_unmatched_null(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_name, b.cost FROM 技能表 a RIGHT JOIN 解锁表 b ON a.skill_id = b.skill_id")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_right_join_from_right_table(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT b.skill_id, b.cost, a.skill_name FROM 解锁表 b RIGHT JOIN 技能表 a ON b.skill_id = a.skill_id")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_right_join_with_where(self, engine, join_fixtures):
        """RIGHT JOIN + WHERE过滤"""
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_name, b.level_req, b.cost FROM 技能表 a RIGHT JOIN 解锁表 b ON a.skill_id = b.skill_id WHERE b.level_req > 3")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_right_join_with_aggregation(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.type, COUNT(*) as cnt FROM 技能表 a RIGHT JOIN 解锁表 b ON a.skill_id = b.skill_id GROUP BY a.type")
        assert result["success"] is True
        rows = _data_rows(result)
        assert len(rows) >= 1
...
class TestFullJoin:
    ...
    def test_basic_full_join(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_id, a.skill_name, b.level_req FROM 技能表 a FULL JOIN 解锁表 b ON a.skill_id = b.skill_id")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_full_join_with_order(self, engine, join_fixtures):
        """FULL JOIN + ORDER BY"""
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_id, a.skill_name, b.level_req FROM 技能表 a FULL JOIN 解锁表 b ON a.skill_id = b.skill_id ORDER BY a.skill_id")
        assert result["success"] is True
        rows = _data_rows(result)
        assert len(rows) == 5
...
class TestCrossJoin:
    ...
    def test_basic_cross_join(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_name, b.cost FROM 技能表 a CROSS JOIN 解锁表 b")
        assert result["success"] is True
        rows = _data_rows(result)
        assert len(rows) == 12
    ...
    def test_cross_join_with_where(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_name, b.cost FROM 技能表 a CROSS JOIN 解锁表 b WHERE a.damage = 200")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_cross_join_with_limit(self, engine, join_fixtures):
        """CROSS JOIN + LIMIT"""
        result = engine.execute_sql_query(join_fixtures, "SELECT a.skill_name, b.cost FROM 技能表 a CROSS JOIN 解锁表 b LIMIT 5")
        assert result["success"] is True
        rows = _data_rows(result)
        assert len(rows) == 5
...
class TestJoinEdgeCases:
    ...
    def test_no_match_right_join(self, engine, tmp_path):
        ...
        result = engine.execute_sql_query(file_path, "SELECT a.name, b.val FROM 技能 a RIGHT JOIN 数据 b ON a.id = b.id")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_single_row_cross_join(self, engine, tmp_path):
        ...
        result = engine.execute_sql_query(file_path, "SELECT * FROM A a CROSS JOIN B b")
        assert result["success"] is True
        rows = _data_rows(result)
        assert len(rows) == 3
    ...
    def test_cross_join_large_cartesian(self, engine, tmp_path):
        ...
        result = engine.execute_sql_query(file_path, "SELECT * FROM L a CROSS JOIN R b")
        assert result["success"] is True
        rows = _data_rows(result)
    ...
    def test_cross_join_with_aggregation(self, engine, join_fixtures):
        ...
        result = engine.execute_sql_query(join_fixtures, "SELECT a.type, SUM(b.cost) as total_cost FROM 技能表 a CROSS JOIN 解锁表 b GROUP BY a.type")
        assert result["success"] is True
        rows = _data_rows(result)
        # 应有法术和物理两个分组
        assert len(rows) >= 1
