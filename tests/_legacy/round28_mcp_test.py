...
def test_update_deep(tr):
    ...
    tr.add("A1-验证: Price=100", r2["success"] and len(r2.get("data", [])) > 1 and r2["data"][1][0] == 100, f"data={r2.get('data', [])}")
    ...
    # A4: UPDATE带CASE WHEN
    r = execute_advanced_update_query(TEST_FILE, "UPDATE 装备配置 SET Price = CASE WHEN Rarity = 'Legendary' THEN 9999 WHEN Rarity = 'Epic' THEN 4999 ELSE 99 END")
...
def test_insert_deep(tr):
    ...
    # C1: 基础INSERT单行
    r = execute_advanced_insert_query(TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (100, 'Excalibur', 150, 50.5, 9999.99, 'Legendary', 'Weapon')")
    ...
    r = execute_advanced_insert_query(
        TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (101, '日本語アイテム「特殊」', 200, 99.9, 888.88, 'Epic', 'Accessory')"
    )
    tr.add("C2: INSERT含Unicode特殊字符", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C3: INSERT含单引号
    r = execute_advanced_insert_query(TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (102, 'It''s Magic', 100, 30.0, 500.0, 'Rare', 'Weapon')")
    tr.add("C3: INSERT含转义单引号(It''s)", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C4: INSERT空字符串值
    r = execute_advanced_insert_query(TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (103, '', 0, 0.0, 0.0, 'Common', '')")
    tr.add("C4: INSERT空字符串值", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C5: INSERT负数值
    r = execute_advanced_insert_query(TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (104, 'DebtItem', -100, -50.5, -999.99, 'Common', 'Weapon')")
    tr.add("C5: INSERT负数值", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C6: INSERT极大数值
    r = execute_advanced_insert_query(
        TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (105, 'BigNum', 2147483647, 999999.999, 999999999.99, 'Legendary', 'Armor')"
    )
    tr.add("C6: INSERT极大数值(INT_MAX级)", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C7: INSERT极小浮点
    r = execute_advanced_insert_query(
        TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (106, 'TinyFloat', 1, 0.000001, 0.000000001, 'Common', 'Accessory')"
    )
    tr.add("C7: INSERT极小浮点数", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # C8: INSERT重复主键(看是否允许)
    r = execute_advanced_insert_query(TEST_FILE, "INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (1, 'Duplicate', 0, 0.0, 0.0, 'Common', 'Test')")
...
def test_case_when_complex(tr):
    ...
    r = execute_advanced_sql_query(TEST_FILE, "SELECT Name, CASE WHEN Rarity = 'Legendary' THEN 'S' WHEN Rarity = 'Epic' THEN 'A' ELSE 'B' END as Grade FROM 装备配置 LIMIT 5")
    ...
    # E2: CASE WHEN嵌套表达式
    r = execute_advanced_sql_query(TEST_FILE, "SELECT Name, CASE WHEN Price > 5000 THEN CASE WHEN BaseAtk > 50 THEN 'TopTier' ELSE 'Expensive' END ELSE 'Normal' END as Tier FROM 装备配置 LIMIT 5")
    tr.add("E2: CASE WHEN嵌套(SELECT)", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # E3: CASE WHEN与聚合函数
    r = execute_advanced_sql_query(TEST_FILE, "SELECT CASE WHEN COUNT(*) > 5 THEN 'Many' ELSE 'Few' END as CntGrade FROM 装备配置")
    tr.add("E3: CASE WHEN包裹聚合COUNT", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # E4: CASE WHEN在WHERE中
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM 装备配置 WHERE CASE WHEN Rarity = 'Legendary' THEN 1 ELSE 0 END = 1 LIMIT 3")
    ...
    r = execute_advanced_sql_query(
        TEST_FILE, "SELECT Name, Rarity, CASE Rarity WHEN 'Legendary' THEN 1 WHEN 'Epic' THEN 2 WHEN 'Rare' THEN 3 ELSE 4 END as ROrder FROM 装备配置 ORDER BY ROrder LIMIT 5"
    )
    tr.add("E5: CASE WHEN简写+ORDER BY", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # E6: CASE WHEN NULL处理(COALESCE替代)
    r = execute_advanced_sql_query(TEST_FILE, "SELECT Name, COALESCE(NULL, Price, 0) as SafePrice FROM 装备配置 LIMIT 3")
    tr.add("E6: COALESCE NULL处理", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # E7: CASE WHEN与算术混合
    r = execute_advanced_sql_query(TEST_FILE, "SELECT Name, BaseAtk * CASE WHEN Rarity = 'Legendary' THEN 2.0 ELSE 1.0 END as AdjAtk FROM 装备配置 LIMIT 5")
    tr.add("E7: CASE WHEN与算术乘法混合", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # E8: 多个CASE WHEN列
    r = execute_advanced_sql_query(
        TEST_FILE, "SELECT Name, CASE WHEN BaseAtk > 50 THEN 'High' ELSE 'Low' END as AtkTier, CASE WHEN Price > 1000 THEN 'Rich' ELSE 'Cheap' END as Wealth FROM 装备配置 LIMIT 5"
    )
...
def test_formula_interaction(tr):
    ...
    r = execute_advanced_sql_query(TEST_FILE_FORMULA, "SELECT f.ID, f.A, p.Value FROM FormulaSheet f JOIN PureData p ON f.ID = p.ID LIMIT 5")
...
def test_known_issues_regression(tr):
    ...
    tr.add("G1: [P0-2回归] SELECT分号多语句", is_injection_working, "仍可注入⚠️" if is_injection_working else "已修复✅")

    # G2: 边界值: 超长字符串
    long_str = "A" * 10000
    # 通过API可能不好传超长SQL，改用插入方式
    r = execute_advanced_insert_query(TEST_FILE, f"INSERT INTO 装备配置 (ID, Name, BaseAtk, AtkBonus, Price, Rarity, Category) VALUES (200, '{long_str}', 1, 1.0, 1.0, 'Common', 'Test')")
...
def test_subquery_deep(tr):
    ...
    # H1: WHERE子查询(标量)
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM 装备配置 WHERE Price > (SELECT AVG(Price) FROM 装备配置) LIMIT 5")
    tr.add("H1: WHERE标量子查询(AVG)", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H2: WHERE IN子查询
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM 装备配置 WHERE ID IN (SELECT ID FROM 装备配置 WHERE Rarity = 'Legendary') LIMIT 5")
    tr.add("H2: WHERE IN子查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H3: FROM子查询(派生表)
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM (SELECT Rarity, AVG(Price) as AvgP FROM 装备配置 GROUP BY Rarity) t WHERE AvgP > 100")
    tr.add("H3: FROM派生表子查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H4: SELECT中的标量子查询
    r = execute_advanced_sql_query(TEST_FILE, "SELECT Name, Price, (SELECT AVG(Price) FROM 装备配置) as OverallAvg FROM 装备配置 LIMIT 3")
    tr.add("H4: SELECT标量子查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H5: CTE + UPDATE (如果支持)
    # 先测试CTE基本功能
    r = execute_advanced_sql_query(TEST_FILE, "WITH HighPrice AS (SELECT * FROM 装备配置 WHERE Price > 5000) SELECT COUNT(*) FROM HighPrice")
    tr.add("H5: CTE基础查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H6: EXISTS相关子查询
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM 装备配置 e WHERE EXISTS (SELECT 1 FROM 装备配置 e2 WHERE e2.Rarity = 'Legendary' AND e2.ID = e.ID) LIMIT 3")
    tr.add("H6: EXISTS相关子查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H7: NOT EXISTS
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM 装备配置 e WHERE NOT EXISTS (SELECT 1 FROM 装备配置 e2 WHERE e2.ID = 99999 AND e2.ID = e.ID) LIMIT 3")
    tr.add("H7: NOT EXISTS子查询", r["success"], r.get("message", "")[:80] if not r["success"] else "")

    # H8: 嵌套多层子查询
    r = execute_advanced_sql_query(TEST_FILE, "SELECT * FROM (SELECT Rarity, AVG(Price) a FROM 装备配置 GROUP BY Rarity) t WHERE a > (SELECT MIN(Price) FROM 装备配置)")
