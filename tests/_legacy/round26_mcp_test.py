...
def create_test_xlsx(path, rows=100, cols=5, sheet_name="Sheet1"):
    ...
    for i in range(1, rows + 1):
        ...
        if cols > 3:
            row.append(round(random.uniform(1.0, 9999.99), 2))
        if cols > 4:
...
try:
    ...
    test("A6: 完全空文件处理", "格式兼容", not r["success"] or r.get("data") is not None, f"success={r.get('success')}, msg={str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("A7: 损坏文件优雅报错", "格式兼容", not r["success"], f"should fail but got: success={r.get('success')}, msg={str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("A8b: 特殊字符Sheet名-查询", "格式兼容", r["success"] and len(r.get("data", [])) <= 3, f"rows: {len(r.get('data', []))}")
except Exception as e:
...
try:
    ...
    test("A9b: Unicode列名+排序", "格式兼容", r["success"] and len(r.get("data", [])) > 0, f"rows: {len(r.get('data', []))}")
except Exception as e:
...
try:
    ...
    test("A9c: Unicode LIKE 查询", "格式兼容", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("A10a: 超长文本读取", "格式兼容", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
    test("A10a: 超长文本读取", "格式兼容", False, str(e)[:100])

try:
    r = execute_advanced_sql_query(longtext_path, "SELECT * FROM LongText WHERE ID = 2")
    test("A10b: 10000字符字段", "格式兼容", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test(f"B1: 10K行 COUNT(*) [{t_query:.2f}s]", "大数据量", r["success"] and cnt == 10000, f"count={cnt}")
except Exception as e:
...
try:
    ...
    t_query = time.time() - t0
    test(f"B2: 10K行 GROUP BY聚合 [{t_query:.2f}s]", "大数据量", r["success"] and len(r.get("data", [])) == 4, f"groups={len(r.get('data', []))}")
except Exception as e:
...
try:
    ...
    t_query = time.time() - t0
    test(f"B3: 10K行 ORDER BY LIMIT10 [{t_query:.2f}s]", "大数据量", r["success"] and len(r.get("data", [])) == 10, f"rows={len(r.get('data', []))}")
except Exception as e:
...
try:
    ...
    test(f"B4: 30列宽表查询 [{t_query:.2f}s]", "大数据量", r["success"], f"cols≈{col_count}")
except Exception as e:
...
try:
    ...
    t_query = time.time() - t0
    test(f"B5: 宽表指定列筛选 [{t_query:.2f}s]", "大数据量", r["success"] and len(r.get("data", [])) <= 10, f"rows={len(r.get('data', []))}")
except Exception as e:
...
try:
    t0 = time.time()
    r = execute_advanced_sql_query(large_path, "SELECT ID, Name, Price, RANK() OVER (ORDER BY Price DESC) as rnk FROM BigTable LIMIT 20")
    t_query = time.time() - t0
    test(f"B6: 10K行 RANK窗口函数 [{t_query:.2f}s]", "大数据量", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
    test("B6: 10K行 RANK窗口函数", "大数据量", False, str(e)[:100])

try:
    t0 = time.time()
    r = execute_advanced_sql_query(large_path, "SELECT Category, Price, ROW_NUMBER() OVER (PARTITION BY Category ORDER BY Price DESC) as rn FROM BigTable WHERE rn <= 3")
    t_query = time.time() - t0
    test(f"B7: 10K行 PARTITION+ROW_NUMBER [{t_query:.2f}s]", "大数据量", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    t_query = time.time() - t0
    test(f"B8: CTE+窗口函数+GROUP BY [{t_query:.2f}s]", "大数据量", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    t0 = time.time()
    r = execute_advanced_update_query(large_path, "UPDATE BigTable SET Value = ROUND(Value * 1.05, 2) WHERE Category = 'A'")
    t_query = time.time() - t0
    test(f"B9: 10K行条件UPDATE [{t_query:.2f}s]", "大数据量", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    t0 = time.time()
    r = execute_advanced_insert_query(large_path, "INSERT INTO BigTable (ID, Name, Value, Price, Category) VALUES (10001, 'StressTest', 999.99, 8888.88, 'Z')")
    t_query = time.time() - t0
    test(f"B10: INSERT新行 [{t_query:.2f}s]", "大数据量", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    if r["success"]:
        ...
        if has_multi or isinstance(r.get("data"), list) and len(r.get("data", [])) > 1:
            test("C1: SELECT分号多语句 [仍存在!]", "P0回归", False, f"P0仍存在! multi-statement executed: {msg_str[:80]}")
        else:
            test("C1: SELECT分号多语句 [已修复?]", "P0回归", True, "单条结果或已拦截")
    else:
        test("C1: SELECT分号多语句 [已拒绝]", "P0回归", True, f"正确拒绝: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("C2: SQL注释符注入", "P0回归", r["success"], f"comment executed: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("C3: 系统路径读取", "P0回归", not r["success"], f"should fail but: success={r.get('success')}, msg={str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("C4: UNION注入", "P0回归", r["success"], f"UNION executed: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D1: 大整数(int32/int64边界)", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D2: 极端浮点数", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D3: NULL值聚合(SUM/COUNT)", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D4: 空字符串与NULL区分", "类型边界", r["success"], f"rows: {len(r.get('data', []))}")
except Exception as e:
...
try:
    ...
    test("D5: 布尔值分组聚合", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D6: 特殊字符值(引号等)", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D7: 日期范围处理", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("D8: 数值溢出表达式", "类型边界", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("E1: 多Sheet顺序查询", "多Sheet", all_ok, f"Players={r1.get('data')}, Guilds={r2.get('data')}, Items={r3.get('data')}")
except Exception as e:
...
try:
    ...
    test("E2: 跨Sheet JOIN统计", "多Sheet", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
...
try:
    ...
    test("E3: CTE跨Sheet关联", "多Sheet", r["success"], f"msg: {str(r.get('message', ''))[:60]}")
except Exception as e:
