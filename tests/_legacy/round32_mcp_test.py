...
def test_group_b_error_recovery(tr):
    ...
    # B1: 语法错误后正常查询能否工作
    f = fresh_file()
    execute_advanced_sql_query(f, "SELECTTTT * FROMM Sheet1")  # 故意语法错误
    ...
    # B2: 表名错误后正确查询
    f2 = fresh_file()
    execute_advanced_sql_query(f2, "SELECT * FROM NonExistentTable")
    ...
    # B3: 列名错误后正确查询
    f3 = fresh_file()
    execute_advanced_sql_query(f3, "SELECT NonExistentCol FROM Sheet1")
    ...
    f4 = fresh_file()
    execute_advanced_update_query(f4, "UPDATTE Sheet1 SETT Value = 999")
    ...
    # B6: 空文件路径错误后正常文件操作
    execute_advanced_sql_query("", "SELECT * FROM Sheet1")
    f6 = fresh_file()
...
def test_group_c_error_quality(tr):
    ...
    for i, (sql, op, expected) in enumerate(test_cases, 1):
        ...
        if has_msg and (has_useful or not is_generic):
            ...
            tr.fail(f"C{i} 错误消息质量: {sql[:30]}", f"msg='{str(msg_orig)[:60]}' (has_msg={has_msg}, useful={has_useful})")
