...
class TestExcelCheckDuplicateIds:
    ...
    def test_no_duplicates_detection(self, no_duplicate_file):
        """测试无重复ID的正确识别"""
        result = ExcelOperations.check_duplicate_ids(str(no_duplicate_file), "技能配置表", id_column=1, header_row=1)
    ...
    def test_duplicates_detection(self, duplicate_file):
        """测试重复ID的正确检测"""
        result = ExcelOperations.check_duplicate_ids(str(duplicate_file), "装备配置表", id_column=1, header_row=1)
    ...
    def test_string_id_duplicates(self, string_id_file):
        """测试字符串ID的重复检测"""
        result = ExcelOperations.check_duplicate_ids(str(string_id_file), "字符串ID表", id_column=1, header_row=1)
    ...
    def test_empty_sheet_handling(self, empty_file):
        """测试空工作表的处理"""
        result = ExcelOperations.check_duplicate_ids(str(empty_file), "空配置表", id_column=1, header_row=1)
    ...
    def test_single_row_handling(self, single_row_file):
        """测试单行数据的处理"""
        result = ExcelOperations.check_duplicate_ids(str(single_row_file), "单行配置表", id_column=1, header_row=1)
    ...
    def test_different_header_row(self, temp_dir):
        ...
        # 使用第2行作为表头
        result = ExcelOperations.check_duplicate_ids(str(file_path), "多表头表", id_column=1, header_row=2)
    ...
    def test_file_not_found(self):
        """测试文件不存在的错误处理"""
        result = ExcelOperations.check_duplicate_ids("nonexistent_file.xlsx", "Sheet1", id_column=1, header_row=1)
    ...
    def test_sheet_not_found(self, no_duplicate_file):
        """测试工作表不存在的错误处理"""
        result = ExcelOperations.check_duplicate_ids(str(no_duplicate_file), "不存在的工作表", id_column=1, header_row=1)
    ...
    def test_large_dataset_performance(self, temp_dir):
        ...
        # 测试大数据集处理
        result = ExcelOperations.check_duplicate_ids(str(file_path), "大数据表", id_column=1, header_row=1)
    ...
    def test_comprehensive_workflow(self, temp_dir):
        ...
        result = ExcelOperations.check_duplicate_ids(str(file_path), "综合测试表", id_column=1, header_row=1)
