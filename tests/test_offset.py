...
class TestOffset:
    ...
    def test_offset_only(self, engine, test_file):
        """OFFSET跳过前N行"""
        result = engine.execute_sql_query(test_file, "SELECT 技能名称, 伤害 FROM 技能配置 ORDER BY 伤害 DESC OFFSET 2")
    ...
    def test_offset_with_limit(self, engine, test_file):
        """OFFSET + LIMIT 分页"""
        result = engine.execute_sql_query(test_file, "SELECT 技能名称, 伤害 FROM 技能配置 ORDER BY 伤害 DESC LIMIT 2 OFFSET 1")
        assert result["success"]
        rows = get_data(result)
        assert len(rows) == 2
        # 跳过雷电120，取火球100和冰箭80
    ...
    def test_offset_zero(self, engine, test_file):
        """OFFSET 0 等于无偏移"""
        result = engine.execute_sql_query(test_file, "SELECT 技能名称 FROM 技能配置 ORDER BY 伤害 DESC LIMIT 2 OFFSET 0")
        assert result["success"]
        rows = get_data(result)
        assert len(rows) == 2
        assert rows[0][0] == "雷电"
    ...
    def test_offset_exceeds_total(self, engine, test_file):
        """OFFSET超过总行数返回空"""
        result = engine.execute_sql_query(test_file, "SELECT 技能名称 FROM 技能配置 OFFSET 999")
        assert result["success"]
        rows = get_data(result)
        assert len(rows) == 0
    ...
    def test_offset_with_where(self, engine, test_file):
        """OFFSET + WHERE组合"""
        result = engine.execute_sql_query(test_file, "SELECT 技能名称, 伤害 FROM 技能配置 WHERE 伤害 > 50 ORDER BY 伤害 DESC OFFSET 1")
        assert result["success"]
        rows = get_data(result)
