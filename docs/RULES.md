...
## 性能优化规则（新增 - REQ-032）
**问题**: 大型Excel文件（2GB+）处理缓慢，影响用户体验
**解决**: 实施分层性能优化策略，确保不同规模文件都有最佳性能
**规则**:
1. **性能基准测试**：每轮必须有性能回归测试
2. **内存使用监控**：实现内存使用量监控和限制机制
3. **流式处理**：大文件必须使用流式处理，避免内存溢出
4. **智能缓存**：对重复查询实现智能缓存，减少重复计算
5. **并发优化**：批量操作使用多线程，但设置合理的线程池大小
6. **测试要求**：
   - 创建 `scripts/performance-benchmark.py` 基准测试脚本
   - 每轮性能测试必须包含小文件、中文件、大文件三种场景
   - 性能退化超过20%时必须回滚并记录到DECISIONS.md
7. **MCP集成**：将性能监控集成到MCP响应中，提供性能报告
🔄 **效率追踪**（2026-04-01 R233，性能优化规则新增 - self-evolve）
**改前基线**: 2GB+文件处理耗时过长，内存占用高，用户体验差
**预期效果**: 大文件处理速度提升3-5倍，内存占用降低50%，用户体验显著改善
